...
export default function App() {
  ...
  if (path.startsWith("/r/")) {
    ...
    const initialStep = step === "book" ? "datetime" : step; // book → datetime
  ...
  if (notFound || !rest) return (
    <div style={{ ...f, minHeight:"100vh", background:"#F8FAFC", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ textAlign:"center", padding:"3rem" }}>
        ...
        <button onClick={() => navigate("/")} style={{ background:"#0F172A", color:"#fff", border:"none", borderRadius:"10px", padding:".8rem 2rem", fontFamily:"inherit", fontSize:".9rem", fontWeight:700, cursor:"pointer" }}>← Ana Səhifə</button>
      </div>
    </div>
  );

  return (
    <div style={{ ...f, background:"#F8FAFC", minHeight:"100vh", color:"#0F172A" }}>
      ...
      {/* Hero */}
      <div style={{ background:`linear-gradient(135deg, ${rest.accent||"#3B82F6"}18 0%, #fff 100%)`, borderBottom:"1px solid #E2E8F0", padding:"2.5rem 2rem" }}>
        <div style={{ maxWidth:"800px", margin:"0 auto", display:"flex", gap:"1.5rem", alignItems:"flex-start", flexWrap:"wrap" }}>
          <div style={{ width:"80px", height:"80px", borderRadius:"20px", background:`${rest.accent||"#3B82F6"}20`, border:`2px solid ${rest.accent||"#3B82F6"}30`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"2.5rem", flexShrink:0 }}>{rest.emoji||"🍽️"}</div>
          <div style={{ flex:1, minWidth:"200px" }}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:".4rem", marginTop:".8rem" }}>
              {rest.tags.map(t => <span key={t} style={{ fontSize:".65rem", padding:".2rem .55rem", borderRadius:"20px", background:`${rest.accent||"#3B82F6"}12`, color:rest.accent||"#3B82F6", border:`1px solid ${rest.accent||"#3B82F6"}25`, fontWeight:500 }}>{t}</span>)}
            </div>
          </div>
          {rest.available && (
            <button onClick={() => navigate(`/r/${toSlug(rest.name)}/book`)}
              style={{ background:"#0F172A", color:"#fff", border:"none", padding:"1rem 2rem", fontFamily:"inherit", fontSize:"1rem", fontWeight:800, cursor:"pointer", borderRadius:"12px", flexShrink:0, boxShadow:"0 4px 16px rgba(0,0,0,.2)" }}>
              🍽️ Rezervasiya Et
            </button>
          )}
        </div>
      ...
      <div style={{ maxWidth:"800px", margin:"0 auto", padding:"2rem" }}>
          ...
          <div style={{ marginBottom:"2rem" }}>
            ...
            <div style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:"14px", overflow:"hidden" }}>
              {cats.map((cat, ci) => (
                <div key={cat}>
                  {ci > 0 && <div style={{ height:"1px", background:"#F1F5F9" }}/>}
                  <div style={{ padding:".6rem 1.2rem", background:"#F8FAFC", fontSize:".62rem", fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:"#94A3B8" }}>{cat}</div>
        ...
        {/* CTA */}
        {rest.available && (
          <div style={{ background:"#0F172A", borderRadius:"16px", padding:"2rem", textAlign:"center", color:"#fff" }}>
            ...
            <button onClick={() => navigate(`/r/${toSlug(rest.name)}/book`)}
              style={{ background:"#3B82F6", color:"#fff", border:"none", padding:".9rem 2.5rem", fontFamily:"inherit", fontSize:"1rem", fontWeight:700, cursor:"pointer", borderRadius:"10px" }}>
              Rezervasiya Et →
            </button>
          </div>
        )}
...
function OnboardingPage({ navigate }) {
  ...
  const lbl = { fontSize:".65rem", fontWeight:600, letterSpacing:".1em", textTransform:"uppercase", color:"#94A3B8", marginBottom:".4rem", display:"block" };
  const inp = { width:"100%", border:"none", background:"transparent", fontSize:".9rem", color:"#0F172A", outline:"none" };
  ...
  return (
    <div style={{ ...f, background:"#F8FAFC", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:"580px", margin:"0 auto", padding:"2.5rem 2rem" }}>
        ...
        <button onClick={submit} disabled={saving}
          style={{ width:"100%", marginTop:"1.5rem", background:saving?"#94A3B8":"#0F172A", color:"#fff", border:"none", padding:"1rem", fontFamily:"inherit", fontSize:"1rem", fontWeight:700, cursor:saving?"not-allowed":"pointer", borderRadius:"10px" }}>
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:"#F8FAFC", minHeight:"100vh", color:"#0F172A" }}>
      ...
      <header style={{ background:"#fff", borderBottom:"1px solid #E2E8F0", position:"sticky", top:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 2rem", height:"60px", boxShadow:"0 1px 3px rgba(0,0,0,.04)" }}>
        ...
        <button onClick={() => navigate("/onboard")} style={{ background:"#0F172A", color:"#fff", border:"none", padding:".4rem .9rem", borderRadius:"8px", fontSize:".78rem", fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
          + Restoran Qeydiyyatı
        </button>
      </header>
...
function MainApp({ navigate, restSlug, initialStep, preTable }) {
  ...
  const stepNum={home:0,table:1,datetime:2,menu:3,success:4}[step]||0;

  const reset=()=>{navigate("/");setSelRest(null);setSelDate("");setSelTime("");setSelTbl(null);setHover(null);setCart({});setGuests(2);setCustName("");setCustPhone("");setBkCode("");setSaveErr("");setBkd([]);setSlots({});setAllRevs([]);};
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#F8FAFC",minHeight:"100vh",color:"#0F172A"}}>
      ...
      {!["home","success","cancel"].includes(step)&&(
        <div style={{background:"#fff",borderBottom:"1px solid #E2E8F0",display:"flex",justifyContent:"center"}}>
          {["Masa Seçimi","Məlumatlarınız","Menyu"].map((s,i)=>{
      ...
      {step==="datetime"&&selRest&&(
        <div style={{maxWidth:"720px",margin:"0 auto",padding:"2rem"}}>
          ...
          <div style={{...box,marginBottom:"1.5rem",display:"flex",gap:"1rem",alignItems:"center",background:"#F0FDF4",border:"1.5px solid #86EFAC"}}>
          ...
          <div style={{fontWeight:800,fontSize:"1.4rem",letterSpacing:"-.02em",marginBottom:".3rem"}}>Məlumatlarınız</div>
          <div style={{fontSize:".78rem",color:"#94A3B8",marginBottom:"1.5rem"}}>Rezervasiyanı tamamlamaq üçün doldurun</div>
          ...
          <button className="btnp" onClick={()=>navigateStep("menu")} disabled={!custName.trim()||!custPhone.trim()}
            style={{background:custName&&custPhone?"#0F172A":"#E2E8F0",color:custName&&custPhone?"#fff":"#94A3B8",border:"none",padding:".9rem 2rem",fontFamily:"inherit",fontSize:".95rem",fontWeight:700,cursor:custName&&custPhone?"pointer":"not-allowed",borderRadius:"10px"}}>
            Menyu → Tamamla
          </button>
        </div>
      )}
      ...
      {step==="table"&&selRest&&(
        <div style={{maxWidth:"960px",margin:"0 auto",padding:"2rem"}}>
          ...
          {/* Restoran başlığı */}
          <div style={{display:"flex",alignItems:"center",gap:".9rem",...box,marginBottom:"1.5rem"}}>
            <div style={{width:"44px",height:"44px",borderRadius:"10px",background:`${selRest.accent||"#3B82F6"}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.4rem",flexShrink:0}}>{selRest.emoji||"🍽️"}</div>
            <div style={{flex:1}}>
              <div style={{fontWeight:800,fontSize:"1.1rem"}}>{selRest.name}</div>
              <div style={{fontSize:".75rem",color:"#94A3B8"}}>{selRest.cuisine} · {selRest.location}</div>
            </div>
            <div style={{fontSize:".75rem",color:"#64748B",textAlign:"right"}}>
              ...
              <input type="date" min={today} value={selDate} onChange={e=>setSelDate(e.target.value)}
                style={{border:"1.5px solid #E2E8F0",borderRadius:"8px",padding:".4rem .7rem",fontSize:".82rem",outline:"none",fontFamily:"inherit",cursor:"pointer",color:"#0F172A"}}/>
            </div>
          </div>
          ...
          {dbTables.length===0?(
            ...
            <>
              {/* Legenda */}
              <div style={{display:"flex",gap:"1.2rem",marginBottom:"1rem",flexWrap:"wrap",alignItems:"center"}}>
                {[{bg:"#F0FDF4",bc:"#86EFAC",l:"Boş"},{bg:"#EFF6FF",bc:"#3B82F6",l:"Seçilmiş"},{bg:"#FEF2F2",bc:"#FCA5A5",l:"Tam dolu"}].map(s=>(
                  <span key={s.l} style={{display:"flex",alignItems:"center",gap:".4rem",fontSize:".72rem",color:"#64748B",fontWeight:500}}>
                    <span style={{width:"14px",height:"14px",background:s.bg,border:`1.5px solid ${s.bc}`,display:"inline-block",borderRadius:"3px"}}/>{s.l}
                  </span>
                ))}
                <span style={{marginLeft:"auto",fontSize:".72rem",color:"#94A3B8"}}>Masaya basın → saat seçin</span>
              </div>

              {/* Masa planı */}
              <div style={{position:"relative",width:"100%",paddingBottom:"50%",background:"#fff",border:"1px solid #E2E8F0",borderRadius:"14px",overflow:"hidden",marginBottom:"1.5rem",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}}>
                <div style={{position:"absolute",inset:0}}>
                  ...
                  {dbTables.map(t=>{
                    ...
                    const isSelected=selTable?.id===t.id, isHovered=hoverTable?.id===t.id;
                    return (
                      <div key={t.id} className="tsvg"
                        style={{position:"absolute",left:`${t.x||10}%`,top:`${t.y||10}%`,transform:"translate(-50%,-50%)",cursor:isFullyBooked?"not-allowed":"pointer",opacity:isFullyBooked?0.45:1,filter:isSelected?"drop-shadow(0 2px 10px rgba(37,99,235,.3))":isHovered?"drop-shadow(0 2px 6px rgba(0,0,0,.12))":"none"}}
                        onClick={()=>{ if(!isFullyBooked){ if(isSelected) setSelTbl(null); else setSelTbl(t); }}}
                        onMouseEnter={()=>setHover(t)} onMouseLeave={()=>setHover(null)}>
                        <TableSVG shape={t.shape||"rect"} label={t.label} seats={t.seats} isBooked={isFullyBooked} isSelected={isSelected} isHovered={isHovered&&!isFullyBooked}/>
                        {/* Boş saat sayı badge */}
                        {selDate&&!isFullyBooked&&(
                          <div style={{position:"absolute",top:"-6px",right:"-6px",background:isSelected?"#2563EB":"#16A34A",color:"#fff",fontSize:".5rem",fontWeight:700,borderRadius:"10px",padding:"1px 5px",lineHeight:1.5,boxShadow:"0 1px 4px rgba(0,0,0,.2)"}}>
                            {freeSlots.length} slot
                          </div>
                        )}
                      </div>
                    );
              ...
              {selTable&&(()=>{
                ...
                return (
                  <div style={{background:"#fff",border:"2px solid #3B82F6",borderRadius:"14px",padding:"1.3rem",marginBottom:"1.2rem",boxShadow:"0 2px 12px rgba(59,130,246,.1)"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"1rem",flexWrap:"wrap",gap:".5rem"}}>
                      <div>
                        <div style={{display:"flex",alignItems:"center",gap:".6rem",marginBottom:".2rem"}}>
                          <span style={{fontWeight:800,fontSize:"1.05rem"}}>🪑 Masa {selTable.label}</span>
                          <span style={{fontSize:".62rem",padding:".15rem .55rem",background:"#EFF6FF",border:"1px solid #BFDBFE",color:"#2563EB",borderRadius:"20px",fontWeight:600}}>Seçildi</span>
                        </div>
                        {selTable.zone&&<div style={{fontSize:".72rem",color:"#64748B"}}>📍 {selTable.zone} · 👥 {selTable.seats} nəfərlik</div>}
                      </div>
                      <button onClick={()=>setSelTbl(null)} style={{background:"none",border:"1px solid #E2E8F0",borderRadius:"6px",padding:".3rem .7rem",fontSize:".72rem",color:"#94A3B8",cursor:"pointer",fontFamily:"inherit"}}>✕ Ləğv et</button>
                    </div>

                    {selTable.img&&<img src={selTable.img} alt={selTable.label} style={{width:"100%",maxHeight:"140px",objectFit:"cover",borderRadius:"8px",marginBottom:"1rem"}} onError={e=>e.target.style.display="none"}/>}
                    ...
                    {!selDate?(
                      ...
                      <>
                        <div style={{fontSize:".72rem",fontWeight:700,color:"#64748B",marginBottom:".6rem",textTransform:"uppercase",letterSpacing:".08em"}}>
                          {selDate} — Boş saatlar ({freeSlots.length} slot · min 30dq, max 1.5 saat)
                        </div>
                        {freeSlots.length===0?(
                          ...
                          <div style={{display:"flex",flexWrap:"wrap",gap:".4rem"}}>
                            {freeSlots.map(slot=>(
                              <button key={slot} onClick={()=>setSelTime(slot)}
                                style={{padding:".5rem .85rem",background:selTime===slot?"#0F172A":"#F8FAFC",color:selTime===slot?"#fff":"#0F172A",border:`1.5px solid ${selTime===slot?"#0F172A":"#E2E8F0"}`,borderRadius:"8px",fontSize:".82rem",fontWeight:selTime===slot?700:500,cursor:"pointer",fontFamily:"inherit",transition:"all .15s"}}>
                                {slot}
                              </button>
                            ))}
                          </div>
              ...
              {!selTable&&(
                <div style={{background:"#F8FAFC",border:"1.5px dashed #CBD5E1",borderRadius:"12px",padding:"1.5rem",textAlign:"center",color:"#94A3B8",marginBottom:"1.2rem"}}>
              ...
              <button className="btnp" onClick={()=>navigateStep("datetime")} disabled={!selTable||!selTime||!selDate}
                style={{background:selTable&&selTime&&selDate?"#0F172A":"#E2E8F0",color:selTable&&selTime&&selDate?"#fff":"#94A3B8",border:"none",padding:".9rem 2rem",fontFamily:"inherit",fontSize:".95rem",fontWeight:700,cursor:selTable&&selTime&&selDate?"pointer":"not-allowed",borderRadius:"10px"}}>
                Davam et — Məlumatlarınızı daxil edin →
              </button>
            </>
          )}
        </div>
