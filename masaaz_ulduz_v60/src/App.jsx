...
// ══════════════════════════════════════════════
// WA credentials — Supabase platform_settings-dən oxunur
let _waCache = null;
async function getWACreds(){
  if(_waCache) return _waCache;
  try{
    const r = await fetch(
      "https://jhyaiuvubbrtngvyoyhi.supabase.co/rest/v1/platform_settings?key=in.(wa_instance,wa_token)&select=key,value",
      {headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpoeWFpdXZ1YmJydG5ndnlveWhpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE2OTg2NTIsImV4cCI6MjA4NzI3NDY1Mn0.UGZhj0rdN9lh7tLnEP1mzFBXlbxFrxWIi7kYUv7_-J0","Content-Type":"application/json"}}
    );
    const rows = await r.json();
    const inst = rows.find(x=>x.key==="wa_instance")?.value||"";
    const tok  = rows.find(x=>x.key==="wa_token")?.value||"";
    _waCache = {inst,tok};
    return _waCache;
  }catch(e){ return {inst:"",tok:""}; }
}

// ── İş saatı yoxlayıcı — gecə yarısını keçən saatları dəstəkləyir
function calcIsOpen(hours) {
  if(!hours) return null;
  const now = new Date();
  const hm = now.getHours()*60 + now.getMinutes();
  const match = hours.replace(/\s/g,"").match(/(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})/);
  if(!match) return null;
  const open = parseInt(match[1])*60 + parseInt(match[2]);
  let close = parseInt(match[3])*60 + parseInt(match[4]);
  // Gecə yarısını keçən saat (məs: 15:00-02:00 → close=120 < open=900)
  if(close <= open) {
    // overnight: açıqdır əgər hm>=open YAXUD hm<close
...
async function sendWA(phone, message){
  const {inst,tok}=await getWACreds();
  if(!inst||!tok) return;
  try{
    ...
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:"#eef3f8", minHeight:"100vh" }}>
      <header style={{ background:"#fff", borderBottom:"1px solid #E2E8F0", height:"60px", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 2rem", boxShadow:"0 1px 3px rgba(0,0,0,.04)" }}>
        <div onClick={() => navigate("/")} style={{ cursor:"pointer", display:"flex", alignItems:"center", gap:".6rem" }}>
          ...
          <rect width="48" height="48" rx="13" fill="#0a1520"/>
          <path d="M14 8 L14 22 Q14 27 19 27 L19 40" stroke="#c8a96e" strokeWidth="2.2" fill="none" strokeLinecap="round"/>
          <path d="M14 8 Q14 16 19 16 Q24 16 24 8" stroke="#c8a96e" strokeWidth="2" fill="none" strokeLinecap="round"/>
          <line x1="29" y1="8" x2="29" y2="40" stroke="#c8a96e" strokeWidth="2.2" strokeLinecap="round"/>
          <path d="M25 8 Q25 18 29 20 Q33 18 33 8" stroke="#c8a96e" strokeWidth="2" fill="none" strokeLinecap="round"/>
        </svg>
        <div style={{fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:"1.05rem",color:"#1a1a2e",lineHeight:1,letterSpacing:"-.02em"}}><span style={{color:"#c8a96e"}}>1</span>rezerv</div>
        </div>
...
// ─── RƏYLƏR SƏHİFƏSİ ────────────────────────────────────────
function ReviewPage({ tableLabel, restSlug }) {
  ...
  const submit=async()=>{
    ...
    setSending(true);
    const restName = resv?.restaurant_name || restSlug || "";
    await supabase.from("reviews").insert({ booking_code:code||null, customer_name:resv?resv.name:(custNm||"Anonim"), restaurant_name:restName, stars, comment:comment.trim()||null, table_label:tableLabel||null });
    setSending(false); setDone(true);
  };
  const f={fontFamily:"'DM Sans',system-ui,sans-serif"};
  const fromQR = !code && tableLabel;
  if(done) return (
    <div style={{...f,minHeight:"100vh",background:"linear-gradient(160deg,#050d17,#0a1f35)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center",padding:"3rem"}}>
        <div style={{fontSize:"4rem",marginBottom:"1rem"}}>🎉</div>
        <div style={{fontSize:"1.6rem",fontWeight:800,color:"#fff",marginBottom:".5rem"}}>Təşəkkür edirik!</div>
        <div style={{color:"rgba(255,255,255,.5)"}}>Rəyiniz qeyd edildi.</div>
        {fromQR && <button onClick={()=>window.history.back()} style={{marginTop:"1.5rem",background:"rgba(255,255,255,.08)",color:"#fff",border:"1px solid rgba(255,255,255,.15)",borderRadius:"10px",padding:".8rem 2rem",fontFamily:"inherit",fontSize:".9rem",fontWeight:600,cursor:"pointer"}}>← Geri</button>}
      </div>
    </div>
  );
  return (
    <div style={{...f,minHeight:"100vh",background:"linear-gradient(160deg,#050d17,#0a1f35)",display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
      <div style={{background:"rgba(255,255,255,.06)",backdropFilter:"blur(20px)",borderRadius:"20px",border:"1px solid rgba(255,255,255,.1)",padding:"2.5rem",maxWidth:"440px",width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:"1.8rem"}}>
          <div style={{fontSize:"2.8rem",marginBottom:".5rem"}}>⭐</div>
          <div style={{fontSize:"1.4rem",fontWeight:800,color:"#fff"}}>Rəyinizi bildirin</div>
          {fromQR
            ? <div style={{fontSize:".82rem",color:"rgba(200,169,110,.7)",marginTop:".4rem"}}>{restSlug} · Masa {tableLabel}</div>
            : resv
              ? <div style={{fontSize:".82rem",color:"rgba(255,255,255,.4)",marginTop:".4rem"}}>{resv.restaurant_name} · {resv.date} · {resv.time}</div>
              : <div style={{fontSize:".75rem",color:"rgba(255,255,255,.35)",marginTop:".3rem"}}>Kod: #{code}</div>
          }
        </div>
        {!resv&&!fromQR&&<div style={{marginBottom:"1rem"}}><label style={{fontSize:".68rem",fontWeight:700,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:".08em",display:"block",marginBottom:".3rem"}}>Adınız</label><input value={custNm} onChange={e=>setCustNm(e.target.value)} placeholder="Əli Həsənov" style={{width:"100%",border:"1px solid rgba(255,255,255,.12)",borderRadius:"8px",padding:".6rem .9rem",fontSize:".9rem",outline:"none",fontFamily:"inherit",background:"rgba(255,255,255,.06)",color:"#fff"}}/></div>}
        <div style={{marginBottom:"1.5rem"}}>
          <div style={{display:"flex",justifyContent:"center",gap:".4rem"}}>
            {[1,2,3,4,5].map(s=><button key={s} onClick={()=>setStars(s)} onMouseEnter={()=>setHov(s)} onMouseLeave={()=>setHov(0)} style={{background:"none",border:"none",cursor:"pointer",fontSize:"2.8rem",color:(hov||stars)>=s?"#F59E0B":"rgba(255,255,255,.15)",transition:"all .1s",transform:(hov||stars)>=s?"scale(1.2)":"scale(1)",padding:"0"}}>★</button>)}
          </div>
          {stars>0&&<div style={{textAlign:"center",fontSize:".88rem",color:"rgba(255,255,255,.5)",marginTop:".6rem",fontWeight:500}}>{["","😞 Pis","😐 Orta","🙂 Yaxşı","😊 Çox yaxşı","🤩 Əla!"][stars]}</div>}
        </div>
        <textarea value={comment} onChange={e=>setComment(e.target.value)} rows={3} placeholder="Restoran, xidmət, yemək haqqında..." style={{width:"100%",border:"1px solid rgba(255,255,255,.1)",borderRadius:"10px",padding:".7rem .9rem",fontSize:".88rem",outline:"none",fontFamily:"inherit",resize:"vertical",lineHeight:1.5,marginBottom:"1rem",background:"rgba(255,255,255,.05)",color:"#fff"}}/>
        {err&&<div style={{color:"#f87171",fontSize:".82rem",marginBottom:".8rem",textAlign:"center"}}>{err}</div>}
        <button onClick={submit} disabled={sending} style={{width:"100%",background:sending?"rgba(255,255,255,.06)":"linear-gradient(135deg,#c8a96e,#e0c07a)",color:sending?"rgba(255,255,255,.3)":"#0a1520",border:"none",borderRadius:"10px",padding:".9rem",fontFamily:"inherit",fontSize:".95rem",fontWeight:700,cursor:sending?"not-allowed":"pointer"}}>
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",minHeight:"100vh",background:"#eef3f8",display:"flex"}}>
      ...
      <aside style={{width:"220px",background:"#12487a",color:"#fff",padding:"1.2rem",flexShrink:0,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
        <div style={{display:"flex",alignItems:"center",gap:".6rem",marginBottom:"2rem",padding:".5rem 0"}}>
          ...
          <div><div style={{fontWeight:800,fontSize:"1rem"}}>1rezerv</div><div style={{fontSize:".58rem",color:"#475569",letterSpacing:".1em"}}>ADMIN</div></div>
        </div>
...
export default function App() {
  ...
  // Favicon + title
  useEffect(() => {
    document.title = "1rezerv — Restoran Rezervasiyası";
    const svg = `<svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="64" height="64" rx="16" fill="%230a1520"/><rect x="12" y="26" width="40" height="20" rx="4" fill="none" stroke="%23c8a96e" stroke-width="2.2"/><rect x="14" y="16" width="10" height="11" rx="3" fill="none" stroke="%23c8a96e" stroke-width="1.8" opacity=".75"/><rect x="40" y="16" width="10" height="11" rx="3" fill="none" stroke="%23c8a96e" stroke-width="1.8" opacity=".75"/><rect x="14" y="45" width="10" height="8" rx="3" fill="none" stroke="%23c8a96e" stroke-width="1.8" opacity=".75"/><rect x="40" y="45" width="10" height="8" rx="3" fill="none" stroke="%23c8a96e" stroke-width="1.8" opacity=".75"/><text x="32" y="41" text-anchor="middle" font-family="Georgia,serif" font-weight="900" font-size="14" fill="%23c8a96e">1</text></svg>`;
  ...
  if (path.startsWith("/r/")) {
    ...
    // /r/:slug/order/:table → QR scan landing (2 seçim)
    if (step === "order" && parts[2]) {
      const qrAction = parts[3]; // "menu" or "review"
      if (qrAction === "menu") {
        return (
          <ErrorBoundary>
            <TableOrderPage navigate={navigate} restSlug={fromSlug(restSlug)} tableLabel={fromSlug(parts[2])} />
          </ErrorBoundary>
        );
      }
      if (qrAction === "review") {
        return <ReviewPage tableLabel={fromSlug(parts[2])} restSlug={fromSlug(restSlug)} />;
      }
      // Default: show landing with 2 choices
      return (
        <ErrorBoundary>
          <QRLandingPage navigate={navigate} restSlug={fromSlug(restSlug)} tableLabel={fromSlug(parts[2])} />
        </ErrorBoundary>
      );
    }
    ...
    // Köhnə QR format: ?table=A3 → yeni QR landing-ə yönləndir
    const tableFromQR = params.get("table");
    if (tableFromQR && !step) {
      return (
        <ErrorBoundary>
          <QRLandingPage navigate={navigate} restSlug={fromSlug(restSlug)} tableLabel={tableFromQR} />
        </ErrorBoundary>
      );
    }
    ...
    <div style={{ ...f, background:"#eef3f8", minHeight:"100vh", color:"#12487a" }}>
        ...
        <div style={{ maxWidth:"800px", margin:"0 auto", display:"flex", gap:"1.5rem", alignItems:"flex-start", flexWrap:"wrap", position:"relative", zIndex:1, padding:rest.cover_url?"2.5rem 2rem":"0", color:rest.cover_url?"#fff":"inherit" }}>
          ...
          <div style={{ flex:1, minWidth:"200px" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:".5rem", marginBottom:".6rem", flexWrap:"wrap" }}>
              <div style={{ fontSize:".85rem", color:"#64748B", textTransform:"uppercase", letterSpacing:".08em" }}>{normCuisine(rest.cuisine)}</div>
              {rest.venue_type && rest.venue_type !== "restoran" && (
                <span style={{ fontSize:".68rem", fontWeight:700, padding:".12rem .5rem", borderRadius:"100px",
                  background: rest.venue_type==="lounge"?"#f0e8ff":rest.venue_type==="pub"?"#fff3e0":"#e8f2fb",
                  color: rest.venue_type==="lounge"?"#7c3aed":rest.venue_type==="pub"?"#c2570a":"#12487a",
                  border: `1px solid ${rest.venue_type==="lounge"?"#c4b5fd":rest.venue_type==="pub"?"#fed7aa":"#a8cce0"}`,
                }}>
                  {{"lounge":"🛋️ Lounge","pub":"🍺 Pub"}[rest.venue_type]}
                </span>
              )}
            </div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:".6rem", fontSize:".8rem", color:"#64748B", marginBottom:".8rem" }}>
              ...
              {rest.discount_percent > 0 && (
                <span className="disc-badge" style={{ display:"inline-flex", alignItems:"center", gap:".3rem", background:"linear-gradient(135deg,#dc2626,#ef4444)", color:"#fff", borderRadius:"10px", padding:".25rem .7rem", fontSize:".75rem", fontWeight:900, boxShadow:"0 3px 10px rgba(220,38,38,.4)" }}>
                  🔥 -{rest.discount_percent}% ENDİRİM
                </span>
              )}
            </div>
            {/* Əlaqə kartları */}
            <div style={{ display:"flex", flexWrap:"wrap", gap:".5rem", marginBottom:".8rem" }}>
              ...
              {(()=>{
                ...
                const isOpen = close <= open ? (hm >= open || hm < close) : (hm >= open && hm < close);
                return (
      ...
      <div style={{ maxWidth:"800px", margin:"0 auto", padding:"2rem" }}>
        ...
        {(rest.phone || rest.hours || rest.location || rest.lat) && (()=>{
          ...
          let isOpen = null;
          if(hoursMatch){ const o=parseInt(hoursMatch[1])*60+parseInt(hoursMatch[2]); const c=parseInt(hoursMatch[3])*60+parseInt(hoursMatch[4]); isOpen = c<=o ? (hmNow>=o||hmNow<c) : (hmNow>=o&&hmNow<c); }
...
function OnboardingPage({ navigate }) {
  const empty = { business_name:"", contact_name:"", phone:"", email:"", venue_type:"restoran", cuisine:"", location:"", table_count:"", message:"" };
      ...
      <div style={{ maxWidth:"580px", margin:"0 auto", padding:"1rem" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:"2rem" }}>
          <div style={{ fontSize:"2.5rem", marginBottom:".8rem" }}>
            {{"restoran":"🍽️","lounge":"🛋️","pub":"🍺"}[form.venue_type]||"🚀"}
          </div>
          <div style={{ fontWeight:800, fontSize:"1.6rem", letterSpacing:"-.02em", marginBottom:".3rem" }}>
            {{"restoran":"Restoranınızı əlavə edin","lounge":"Lounge-unuzu əlavə edin","pub":"Pub-ınızı əlavə edin"}[form.venue_type]||"Obyektinizi əlavə edin"}
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:".75rem", marginBottom:"1.5rem" }}>
          {[
            { key:"restoran", icon:"🍽️", label:"Restoran", desc:"Nahar, şam yeməyi" },
            { key:"lounge",   icon:"🛋️", label:"Lounge",   desc:"Bar, gecə klubu" },
            { key:"pub",      icon:"🍺", label:"Pub",      desc:"İçki, əyləncə" },
          ].map(v => (
            <div key={v.key} onClick={() => setForm(p => ({...p, venue_type: v.key}))}
              style={{
                background: form.venue_type===v.key ? "#0a1f35" : "#fff",
                border: `2px solid ${form.venue_type===v.key ? "#c8a96e" : "#E2E8F0"}`,
                borderRadius:"14px", padding:"1rem .75rem", textAlign:"center",
                cursor:"pointer", transition:"all .18s"
              }}>
              <div style={{ fontSize:"1.6rem", marginBottom:".35rem" }}>{v.icon}</div>
              <div style={{ fontWeight:700, fontSize:".85rem", color: form.venue_type===v.key ? "#fff" : "#1a1a2e", marginBottom:".2rem" }}>{v.label}</div>
              <div style={{ fontSize:".65rem", color: form.venue_type===v.key ? "rgba(255,255,255,.6)" : "#94A3B8" }}>{v.desc}</div>
            </div>
          ))}
        </div>
...
// ══════════════════════════════════════════════
// 📱 PHONE DEMO MOCKUP — Hero animasiyası
// ══════════════════════════════════════════════

// ════════════════════════════════════════════════════════════

// ════════════════════════════════════════════════════════════
// 📱 QR LANDING  →  /r/:slug/order/:table
// Müştəri QR oxudur → 2 seçim: Menyu | Dəyərləndirmə
// ════════════════════════════════════════════════════════════
function QRLandingPage({ navigate, restSlug, tableLabel }) {
  const [rest, setRest] = useState(null);
  useEffect(() => {
    supabase.from("restaurants").select("name,cover_url,accent,emoji").ilike("name", restSlug).maybeSingle()
      .then(({ data }) => setRest(data));
  }, [restSlug]);

  const ac = rest?.accent || "#c8a96e";

  return (
    <div style={{
      minHeight:"100vh",
      background:"linear-gradient(160deg,#050d17,#0a1f35,#0f2d4a)",
      ...
      {/* Logo */}
      <div style={{marginBottom:"1.5rem"}}>
        <div style={{fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:"1.4rem",color:"#fff"}}>
          <span style={{color:"#c8a96e"}}>1</span>rezerv
        </div>
      </div>

      {/* Restoran + Masa */}
      <div style={{marginBottom:"2.5rem"}}>
        <div style={{fontFamily:"'Playfair Display',serif",fontWeight:800,fontSize:"1.8rem",color:"#fff",marginBottom:".5rem"}}>
          {rest?.name || restSlug}
        </div>
        <div style={{
          display:"inline-flex", alignItems:"center", gap:".5rem",
          background:"rgba(200,169,110,.12)", border:"1px solid rgba(200,169,110,.3)",
          borderRadius:"100px", padding:".4rem 1.2rem"
        }}>
          <span style={{fontSize:"1rem"}}>🪑</span>
          <span style={{color:"#c8a96e", fontWeight:700, fontSize:"1rem"}}>Masa {tableLabel}</span>
        </div>
      </div>
      ...
      <div style={{display:"flex", flexDirection:"column", gap:"1rem", width:"100%", maxWidth:"320px"}}>
        {/* Menyu */}
        <button
          onClick={() => navigate(`/r/${encodeURIComponent(restSlug)}/order/${encodeURIComponent(tableLabel)}/menu`)}
          style={{
            background:"linear-gradient(135deg,#c8a96e,#e0c07a)",
            color:"#0a1520", border:"none", borderRadius:"16px",
            padding:"1.4rem 1.5rem",
            display:"flex", alignItems:"center", gap:"1rem",
            cursor:"pointer", fontFamily:"inherit", textAlign:"left",
            boxShadow:"0 8px 28px rgba(200,169,110,.35)", transition:"transform .15s"
          }}
          onMouseEnter={e=>e.currentTarget.style.transform="scale(1.02)"}
          onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}
        >
          <div style={{width:"52px",height:"52px",borderRadius:"12px",background:"rgba(10,21,32,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.8rem",flexShrink:0}}>🍽️</div>
          <div>
            <div style={{fontWeight:800,fontSize:"1.05rem",marginBottom:".2rem"}}>Sifariş Ver</div>
            <div style={{fontSize:".8rem",opacity:.7}}>Menyudan seçin, sifarişiniz masanıza gəlsin</div>
          </div>
        </button>

        {/* Dəyərləndirmə */}
        <button
          onClick={() => navigate(`/r/${encodeURIComponent(restSlug)}/order/${encodeURIComponent(tableLabel)}/review`)}
          style={{
            background:"rgba(255,255,255,.06)", color:"#fff",
            border:"1.5px solid rgba(255,255,255,.15)", borderRadius:"16px",
            padding:"1.4rem 1.5rem",
            display:"flex", alignItems:"center", gap:"1rem",
            cursor:"pointer", fontFamily:"inherit", textAlign:"left",
          ...
          onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}
        >
          <div style={{width:"52px",height:"52px",borderRadius:"12px",background:"rgba(255,255,255,.08)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.8rem",flexShrink:0}}>⭐</div>
          <div>
            <div style={{fontWeight:700,fontSize:"1.05rem",marginBottom:".2rem"}}>Dəyərləndirin</div>
            <div style={{fontSize:".8rem",color:"rgba(255,255,255,.45)"}}>Xidmət haqqında rəyinizi bildirin</div>
          </div>
        </button>

        {/* Çıxış */}
        <ExitButton restSlug={restSlug} tableLabel={tableLabel} />
      </div>

      <div style={{marginTop:"2.5rem",fontSize:".72rem",color:"rgba(255,255,255,.2)",letterSpacing:".05em"}}>
        1rezerv.az — Rəqəmsal Restoran Platforması
      </div>
    </div>
  );
}

// 🚪 ÇIXIŞ DÜYMƏSİ — Müştəri restoranı tərk edir
function ExitButton({ restSlug, tableLabel }) {
  ...
  const handleExit = async () => {
    ...
    try {
      ...
      const { data } = await supabase
        .from("reservations")
        .select("id,status")
        .ilike("restaurant_name", restSlug)
        .eq("table_label", tableLabel)
        .eq("date", today)
        .in("status", ["gözlənilir","təsdiqləndi","gəldi"])
        .order("created_at", { ascending: false })
        .limit(1);
      if (data && data.length > 0) {
        await supabase.from("reservations").update({ status: "restoranı tərk etdi" }).eq("id", data[0].id);
        setDone(true);
      } else {
        setError("Aktiv rezervasiya tapılmadı");
      }
  ...
  if (done) return (
    <div style={{ background:"rgba(34,197,94,.12)", border:"1.5px solid rgba(34,197,94,.3)", borderRadius:"16px", padding:"1.4rem 1.5rem", textAlign:"center", color:"#4ade80" }}>
      <div style={{fontSize:"2rem",marginBottom:".5rem"}}>✅</div>
      <div style={{fontWeight:700}}>Çıxışınız qeyd edildi</div>
      <div style={{fontSize:".8rem",opacity:.7,marginTop:".3rem"}}>Yaxında görüşmək üçün!</div>
    </div>
  );
    ...
    <div>
      <button
        ...
        style={{
          ...
          border:"1.5px solid rgba(255,255,255,.1)", borderRadius:"16px",
          padding:"1rem 1.5rem", width:"100%",
          display:"flex", alignItems:"center", gap:"1rem",
          cursor: loading ? "not-allowed" : "pointer",
          fontFamily:"inherit", textAlign:"left", transition:"transform .15s"
        }}
        onMouseEnter={e=>!loading && (e.currentTarget.style.transform="scale(1.02)")}
        onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}
      >
        <div style={{width:"52px",height:"52px",borderRadius:"12px",background:"rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.8rem",flexShrink:0}}>🚪</div>
        <div>
          <div style={{fontWeight:700,fontSize:"1.05rem",marginBottom:".2rem",color:"rgba(255,255,255,.7)"}}>
            {loading ? "⏳ Qeyd edilir..." : "Restoranı tərk et"}
          </div>
          <div style={{fontSize:".8rem",color:"rgba(255,255,255,.3)"}}>Masanı boşaltdığınızı bildirin</div>
        </div>
      </button>
      {error && <div style={{color:"#f87171",fontSize:".78rem",textAlign:"center",marginTop:".5rem"}}>{error}</div>}
    </div>
  );
}

// 🍽️ MASA SİFARİŞİ SƏHİFƏSİ  →  /r/:slug/order/:table
// Müştəri masada QR oxudur → menyu seçir → admin-ə gedir
// ════════════════════════════════════════════════════════════
function TableOrderPage({ navigate, restSlug, tableLabel }) {
  const [rest, setRest] = useState(null);
  ...
  useEffect(() => {
    supabase.from("restaurants").select("*").ilike("name", restSlug).maybeSingle().then(({ data: r }) => {
      if (!r) return;
      setRest(r);
      supabase.from("menu_items").select("*").eq("restaurant_id", r.id).eq("available", true).order("sort_order")
        .then(({ data: mi }) => {
          ...
          else if (Array.isArray(r.menu_items)) setMenuItems(r.menu_items);
        });
    });
  }, [restSlug]);

  const [selectedItem, setSelectedItem] = useState(null); // for image modal
  const cats = useMemo(() => [...new Set(menuItems.map(i => i.cat).filter(Boolean))], [menuItems]);
  const filtered = activeCat === "Hamısı" ? menuItems : menuItems.filter(i => i.cat === activeCat);
  const addItem = item => setCart(p => ({ ...p, [item.id]: (p[item.id]||0)+1 }));
  const removeItem = item => setCart(p => { const n={...p}; if((n[item.id]||0)<=1) delete n[item.id]; else n[item.id]--; return n; });
  const cartItems = menuItems.filter(i => cart[i.id]);
  const cartTotal = cartItems.reduce((s,i) => s+(cart[i.id]||0)*i.price, 0);
  const cartCount = cartItems.reduce((s,i) => s+(cart[i.id]||0), 0);

  const submitOrder = async () => {
    if(cartCount===0) return;
    setSending(true);
    const items = cartItems.map(i => ({ name:i.name, qty:cart[i.id], price:i.price, emoji:i.emoji||"🍽️" }));
  ...
  if(sent) return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#050d17,#0a1f35)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem",textAlign:"center",fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      <div style={{width:"80px",height:"80px",borderRadius:"50%",background:"rgba(34,197,94,.12)",border:"2.5px solid rgba(34,197,94,.45)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2.5rem",marginBottom:"1.2rem",color:"#4ade80"}}>✓</div>
      <div style={{fontFamily:"'Playfair Display',serif",fontWeight:800,fontSize:"1.6rem",color:"#fff",marginBottom:".5rem"}}>Sifariş Qəbul Edildi!</div>
      <div style={{color:"rgba(255,255,255,.45)",fontSize:".9rem",marginBottom:".5rem"}}>
        <span style={{background:"rgba(200,169,110,.15)",border:"1px solid rgba(200,169,110,.3)",borderRadius:"8px",padding:".2rem .7rem",color:"#c8a96e",fontWeight:700}}>Masa {tableLabel}</span>
      </div>
      <div style={{color:"rgba(255,255,255,.35)",fontSize:".82rem",marginBottom:"2rem"}}>{rest?.name}</div>
      <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:"14px",padding:"1.2rem 1.5rem",marginBottom:"1.5rem",minWidth:"280px",width:"100%",maxWidth:"360px"}}>
        {cartItems.map(item=>(
          <div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:".4rem 0",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
            <span style={{color:"rgba(255,255,255,.75)",fontSize:".88rem"}}>{item.emoji||"🍽️"} {item.name} ×{cart[item.id]}</span>
            <span style={{color:"#c8a96e",fontWeight:700,fontSize:".88rem"}}>{(cart[item.id]*item.price).toFixed(2)}₼</span>
          </div>
        ))}
        <div style={{display:"flex",justifyContent:"space-between",marginTop:".6rem",paddingTop:".4rem"}}>
          <span style={{color:"rgba(255,255,255,.5)",fontWeight:600}}>Cəmi</span>
          <span style={{color:"#c8a96e",fontWeight:800,fontSize:"1rem"}}>{cartTotal.toFixed(2)}₼</span>
        </div>
      </div>
      <div style={{color:"rgba(255,255,255,.3)",fontSize:".8rem",marginBottom:"1.5rem"}}>⏳ Sifarişiniz hazırlanır...</div>
      <button onClick={()=>{setSent(false);setCart({});setNote("");}}
        style={{background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.12)",color:"rgba(255,255,255,.6)",borderRadius:"10px",padding:".75rem 2rem",fontFamily:"inherit",fontSize:".88rem",cursor:"pointer"}}>
        Yenidən sifariş ver
      </button>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#050d17,#0a1f35,#0f2d4a)",fontFamily:"'DM Sans',system-ui,sans-serif",color:"#fff",paddingBottom:"120px"}}>
      {/* Header */}
      <div style={{background:"rgba(5,13,23,.95)",backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:100,borderBottom:"1px solid rgba(255,255,255,.07)"}}>
        <div style={{padding:".9rem 1.2rem 0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontFamily:"'Playfair Display',serif",fontWeight:800,fontSize:"1.1rem",color:"#fff"}}>{rest?.name||restSlug}</div>
            <div style={{fontSize:".72rem",color:"rgba(200,169,110,.7)",marginTop:"1px"}}>🪑 Masa <strong style={{color:"#c8a96e"}}>{tableLabel}</strong> · Sifariş</div>
          </div>
          {cartCount>0&&(
            <div style={{background:"linear-gradient(135deg,#c8a96e,#e0c07a)",color:"#0a1520",borderRadius:"20px",padding:".35rem .9rem",fontSize:".82rem",fontWeight:700}}>
        ...
        {/* Category tabs */}
        {cats.length>0&&(
          <div style={{display:"flex",gap:".4rem",padding:".7rem 1rem",overflowX:"auto"}}>
            {["Hamısı",...cats].map(cat=>(
              <button key={cat} onClick={()=>setActiveCat(cat)}
                style={{background:activeCat===cat?"rgba(200,169,110,.18)":"rgba(255,255,255,.04)",border:`1px solid ${activeCat===cat?"rgba(200,169,110,.45)":"rgba(255,255,255,.08)"}`,color:activeCat===cat?"#c8a96e":"rgba(255,255,255,.45)",borderRadius:"100px",padding:".35rem .85rem",fontSize:".75rem",fontWeight:activeCat===cat?700:400,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit"}}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <div style={{padding:"1rem",maxWidth:"600px",margin:"0 auto"}}>
        {menuItems.length===0?(
          <div style={{textAlign:"center",padding:"5rem 2rem",color:"rgba(255,255,255,.3)"}}>
            <div style={{fontSize:"3rem",marginBottom:"1rem"}}>🍽️</div>
            <div style={{fontWeight:600,fontSize:"1rem"}}>Menyu yüklənir...</div>
          </div>
        ):filtered.map(item=>(
          <div key={item.id} style={{background:cart[item.id]?"rgba(200,169,110,.08)":"rgba(255,255,255,.04)",border:`1.5px solid ${cart[item.id]?"rgba(200,169,110,.3)":"rgba(255,255,255,.07)"}`,borderRadius:"14px",marginBottom:".7rem",overflow:"hidden",transition:"all .2s"}}>
            {/* Image — clickable */}
            {item.image_url||item.img_url ? (
              <div onClick={()=>setSelectedItem(item)} style={{position:"relative",height:"160px",cursor:"pointer",overflow:"hidden"}}>
                <img src={item.image_url||item.img_url} alt={item.name} style={{width:"100%",height:"100%",objectFit:"cover",display:"block",transition:"transform .3s"}}
                ...
                <div style={{position:"absolute",bottom:"10px",left:"12px",right:"12px",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                  ...
                  <div style={{fontWeight:800,color:"#c8a96e",fontSize:"1rem",background:"rgba(10,21,32,.7)",borderRadius:"8px",padding:".2rem .6rem"}}>{item.price}₼</div>
                </div>
                <div style={{position:"absolute",top:"10px",right:"10px",background:"rgba(0,0,0,.5)",borderRadius:"100px",padding:".2rem .6rem",fontSize:".65rem",color:"rgba(255,255,255,.7)"}}>👁 Bax</div>
              </div>
            ) : null}
            {/* Body */}
            <div style={{padding:item.image_url||item.img_url?".7rem 1rem 1rem":"1rem",display:"flex",alignItems:"center",gap:"1rem"}}>
              {!(item.image_url||item.img_url) && (
                <div onClick={()=>setSelectedItem(item)} style={{width:"72px",height:"72px",borderRadius:"10px",background:"rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2.2rem",flexShrink:0,cursor:"pointer"}}>{item.emoji||"🍽️"}</div>
              )}
              <div style={{flex:1,minWidth:0}}>
                {!(item.image_url||item.img_url) && <div style={{fontWeight:700,fontSize:".95rem",color:"rgba(255,255,255,.9)",marginBottom:".2rem"}}>{item.name}</div>}
                {item.desc&&<div style={{fontSize:".75rem",color:"rgba(255,255,255,.35)",marginBottom:".4rem",lineHeight:1.4}}>{item.desc}</div>}
                {!(item.image_url||item.img_url) && <div style={{fontWeight:800,color:"#c8a96e",fontSize:".92rem"}}>{item.price}₼</div>}
              </div>
              <div style={{display:"flex",alignItems:"center",gap:".4rem",flexShrink:0}}>
                {cart[item.id]?(
                  <>
                    <button onClick={()=>removeItem(item)} style={{width:"34px",height:"34px",background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.12)",color:"rgba(255,255,255,.7)",borderRadius:"9px",fontSize:"1.3rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>−</button>
                    <span style={{minWidth:"24px",textAlign:"center",fontWeight:800,fontSize:"1rem",color:"#c8a96e"}}>{cart[item.id]}</span>
                  </>
                ):null}
                <button onClick={()=>addItem(item)} style={{width:"34px",height:"34px",background:cart[item.id]?"rgba(200,169,110,.25)":"rgba(200,169,110,.12)",border:"1.5px solid rgba(200,169,110,.4)",color:"#c8a96e",borderRadius:"9px",fontSize:"1.3rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>+</button>
              </div>
            </div>
          </div>
        ))}
        ...
        {cartCount>0&&(
          <div style={{marginTop:".5rem",marginBottom:"1rem"}}>
            <textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Xüsusi istək, allergi qeydi... (istəyə görə)" rows={2}
              style={{width:"100%",background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:"10px",padding:".75rem 1rem",color:"#fff",fontFamily:"inherit",fontSize:".85rem",outline:"none",resize:"none"}}/>
          </div>
        )}
      </div>
      ...
      {/* ── Item Detail Modal ── */}
      {selectedItem && (
        <div onClick={()=>setSelectedItem(null)} style={{position:"fixed",inset:0,background:"rgba(5,13,23,.92)",zIndex:999,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"linear-gradient(160deg,#0a1f35,#0f2d4a)",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:"600px",maxHeight:"90vh",overflow:"auto"}}>
            {/* Image */}
            {(selectedItem.image_url||selectedItem.img_url) && (
              <div style={{height:"260px",overflow:"hidden",borderRadius:"20px 20px 0 0",position:"relative"}}>
                <img src={selectedItem.image_url||selectedItem.img_url} alt={selectedItem.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,rgba(10,21,32,.8))"}}/>
                <button onClick={()=>setSelectedItem(null)} style={{position:"absolute",top:"12px",right:"12px",width:"34px",height:"34px",borderRadius:"50%",background:"rgba(0,0,0,.5)",border:"none",color:"#fff",fontSize:"1.1rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
              </div>
            )}
            <div style={{padding:"1.5rem"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:".8rem"}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontWeight:800,fontSize:"1.4rem",color:"#fff",flex:1,paddingRight:"1rem"}}>{selectedItem.name}</div>
                <div style={{fontWeight:900,fontSize:"1.4rem",color:"#c8a96e",whiteSpace:"nowrap"}}>{selectedItem.price}₼</div>
              </div>
              {selectedItem.desc && <div style={{fontSize:".9rem",color:"rgba(255,255,255,.5)",lineHeight:1.6,marginBottom:"1.2rem"}}>{selectedItem.desc}</div>}
              <div style={{display:"flex",alignItems:"center",gap:".8rem"}}>
                {cart[selectedItem.id]?(
                  <>
                    <button onClick={()=>removeItem(selectedItem)} style={{width:"44px",height:"44px",background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.15)",color:"rgba(255,255,255,.7)",borderRadius:"11px",fontSize:"1.4rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                    <span style={{fontWeight:800,fontSize:"1.2rem",color:"#c8a96e",minWidth:"28px",textAlign:"center"}}>{cart[selectedItem.id]}</span>
                  </>
                ):null}
                <button onClick={()=>{addItem(selectedItem);setSelectedItem(null);}}
                  style={{flex:1,background:"linear-gradient(135deg,#c8a96e,#e0c07a)",color:"#0a1520",border:"none",borderRadius:"12px",padding:"1rem",fontFamily:"inherit",fontSize:"1rem",fontWeight:800,cursor:"pointer",boxShadow:"0 6px 20px rgba(200,169,110,.4)"}}>
                  + Səbətə əlavə et
                </button>
              </div>
            </div>
      ...
      {cartCount>0&&(
        <div style={{position:"fixed",bottom:0,left:0,right:0,padding:"1rem 1.2rem",background:"rgba(5,13,23,.95)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,.07)"}}>
          <button onClick={submitOrder} disabled={sending}
            style={{width:"100%",background:sending?"rgba(255,255,255,.1)":"linear-gradient(135deg,#c8a96e,#e0c07a)",color:sending?"rgba(255,255,255,.4)":"#0a1520",border:"none",borderRadius:"12px",padding:"1rem",fontFamily:"inherit",fontSize:"1rem",fontWeight:800,cursor:sending?"not-allowed":"pointer",boxShadow:sending?"none":"0 8px 24px rgba(200,169,110,.4)",transition:"all .2s"}}>
            {sending ? "⏳ Göndərilir..." : `✓ Sifariş Ver — ${cartTotal.toFixed(2)}₼`}
          </button>
        </div>
      )}
    </div>
...
function PhoneDemoMockup() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setFade(false);
      setTimeout(() => { setScreenIdx(s => (s + 1) % 4); setFade(true); }, 300);
    }, 3500);
    return () => clearInterval(t);
  }, []);

  const SB = () => (
    <div style={{height:"28px",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px 0"}}>
      <span style={{fontSize:".5rem",color:"rgba(255,255,255,.6)",fontWeight:600}}>9:41</span>
      <div style={{display:"flex",gap:"4px",alignItems:"center"}}>
        <span style={{fontSize:".5rem",color:"rgba(255,255,255,.5)"}}>●●●</span>
      </div>
    </div>
  );
  ...
  const screens = [
    // Screen 1: Ana səhifə
    <div key="home" style={{height:"100%",background:"linear-gradient(160deg,#050d17,#0a1f35)"}}>
      <SB/>
      <div style={{padding:"0 12px 10px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"14px"}}>
          <div style={{width:"22px",height:"22px",borderRadius:"6px",background:"#0a1520",border:"1px solid rgba(200,169,110,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:".6rem"}}>🍽</div>
          <span style={{fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:".75rem",color:"#fff"}}><span style={{color:"#c8a96e"}}>1</span>rezerv</span>
        </div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:".95rem",fontWeight:900,color:"#fff",lineHeight:1.2,marginBottom:"2px"}}>Masanızı elə indi</div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:".95rem",fontWeight:900,color:ac,marginBottom:"12px"}}>rezerv edin</div>
        <div style={{background:"rgba(255,255,255,.08)",borderRadius:"9px",padding:"6px 10px",fontSize:".55rem",color:"rgba(255,255,255,.4)",marginBottom:"10px",display:"flex",alignItems:"center",gap:"5px"}}>🔍 Restoran, mətbəx axtar...</div>
        {[{e:"🍽️",n:"Buxar",l:"Saray, Abşeron",c:"#1a5c8a",rat:"4.8"},{e:"🥩",n:"Steak House",l:"Neftçilər",c:"#7c3030",rat:"4.5"}].map((r,i)=>(
          <div key={i} style={{background:i===0?"rgba(200,169,110,.1)":"rgba(255,255,255,.05)",border:`1px solid ${i===0?"rgba(200,169,110,.3)":"rgba(255,255,255,.08)"}`,borderRadius:"11px",padding:"7px 9px",marginBottom:"6px",display:"flex",alignItems:"center",gap:"7px"}}>
            <div style={{width:"28px",height:"28px",borderRadius:"8px",background:`linear-gradient(135deg,${r.c},${r.c}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:".85rem",flexShrink:0}}>{r.e}</div>
            <div style={{flex:1}}>
              <div style={{fontWeight:700,fontSize:".65rem",color:"#fff"}}>{r.n}</div>
              <div style={{fontSize:".5rem",color:"rgba(255,255,255,.4)",marginTop:"1px"}}>📍 {r.l}</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"3px"}}>
              <div style={{fontSize:".45rem",color:"#c8a96e",fontWeight:700}}>⭐ {r.rat}</div>
              <div style={{background:ac,color:"#0a1520",borderRadius:"5px",padding:"2px 6px",fontSize:".5rem",fontWeight:800}}>Rezerv</div>
            </div>
          </div>
        ))}
      </div>
    ...
    // Screen 2: Masa seçimi — şəkilə uyğun
    <div key="tables" style={{height:"100%",background:"linear-gradient(160deg,#050d17,#0a1f35,#0f2d4a)"}}>
      <SB/>
      <div style={{padding:"0 9px 8px"}}>
        {/* Discount banner */}
        <div style={{background:"linear-gradient(135deg,rgba(220,38,38,.2),rgba(239,68,68,.1))",border:"1px solid rgba(220,38,38,.3)",borderRadius:"8px",padding:"5px 8px",marginBottom:"7px",display:"flex",alignItems:"center",gap:"5px"}}>
          <span style={{fontSize:".7rem"}}>🔥</span>
          <div>
            <div style={{fontWeight:800,fontSize:".55rem",color:"#f87171"}}>-5% ENDİRİM AKTİVDİR!</div>
            <div style={{fontSize:".45rem",color:"rgba(255,255,255,.4)"}}>Bütün menyu maddələrinə tətbiq edilir</div>
          </div>
        </div>
        {/* Filter chips */}
        <div style={{display:"flex",gap:"4px",marginBottom:"8px"}}>
          {["Boş","Seçilmiş","Tam dolu"].map((l,i)=>(
            <div key={l} style={{borderRadius:"100px",padding:"2px 7px",fontSize:".48rem",fontWeight:600,background:i===0?"rgba(34,197,94,.12)":i===1?"rgba(200,169,110,.2)":"rgba(239,68,68,.12)",border:`1px solid ${i===0?"rgba(34,197,94,.35)":i===1?"rgba(200,169,110,.5)":"rgba(239,68,68,.35)"}`,color:i===0?"#4ade80":i===1?"#c8a96e":"#f87171"}}>{l}</div>
          ))}
        </div>
        {/* Table grid */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px",marginBottom:"6px"}}>
          {[{l:"A1,B2,",s:4,z:"VIP",st:"free"},{l:"A2",s:10,st:"free"},{l:"A3",s:2,z:"Terrace",st:"free"},{l:"A4",s:1,st:"selected"},{l:"A5",s:5,st:"free"}].map((t,i)=>(
            <div key={t.l} style={{background:t.st==="selected"?"rgba(200,169,110,.15)":t.st==="booked"?"rgba(239,68,68,.08)":"rgba(255,255,255,.05)",border:`1.5px solid ${t.st==="selected"?"rgba(200,169,110,.7)":t.st==="booked"?"rgba(239,68,68,.3)":"rgba(255,255,255,.1)"}`,borderRadius:"10px",padding:"8px 6px",textAlign:"center",position:"relative",gridColumn:i===4?"span 2":undefined}}>
              {t.st==="selected"&&<div style={{position:"absolute",top:"-5px",right:"-5px",width:"14px",height:"14px",borderRadius:"50%",background:"#c8a96e",display:"flex",alignItems:"center",justifyContent:"center",fontSize:".45rem",color:"#0a1520",fontWeight:900}}>✓</div>}
              <svg width="26" height="17" viewBox="0 0 26 17" style={{margin:"0 auto 3px",display:"block"}}>
                <rect x="2" y="4" width="22" height="9" rx="2" fill="none" stroke={t.st==="selected"?"#c8a96e":t.st==="booked"?"rgba(239,68,68,.5)":"rgba(255,255,255,.25)"} strokeWidth="1.2"/>
                <rect x="0" y="1" width="5" height="4" rx="1.5" fill="none" stroke={t.st==="selected"?"rgba(200,169,110,.5)":"rgba(255,255,255,.15)"} strokeWidth=".8"/>
                <rect x="21" y="1" width="5" height="4" rx="1.5" fill="none" stroke={t.st==="selected"?"rgba(200,169,110,.5)":"rgba(255,255,255,.15)"} strokeWidth=".8"/>
                <rect x="0" y="12" width="5" height="4" rx="1.5" fill="none" stroke={t.st==="selected"?"rgba(200,169,110,.5)":"rgba(255,255,255,.15)"} strokeWidth=".8"/>
                <rect x="21" y="12" width="5" height="4" rx="1.5" fill="none" stroke={t.st==="selected"?"rgba(200,169,110,.5)":"rgba(255,255,255,.15)"} strokeWidth=".8"/>
              </svg>
              <div style={{fontWeight:800,fontSize:".52rem",color:t.st==="selected"?"#c8a96e":t.st==="booked"?"rgba(239,68,68,.7)":"rgba(255,255,255,.75)"}}>{t.l}</div>
              <div style={{fontSize:".44rem",color:"rgba(255,255,255,.35)",marginTop:"1px"}}>👥 {t.s} nf{t.z?` · ${t.z}`:""}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    // Screen 3: Saat seçimi
    <div key="time" style={{height:"100%",background:"linear-gradient(160deg,#050d17,#0a1f35,#0f2d4a)"}}>
      <SB/>
      <div style={{padding:"0 9px 8px"}}>
        <div style={{background:"rgba(200,169,110,.1)",border:"1.5px solid rgba(200,169,110,.3)",borderRadius:"9px",padding:"6px 9px",marginBottom:"7px",display:"flex",alignItems:"center",gap:"6px"}}>
          <span style={{fontSize:".75rem"}}>🪑</span>
          <div>
            <div style={{fontWeight:700,fontSize:".62rem",color:"#c8a96e"}}>Masa A4 seçildi</div>
            <div style={{fontSize:".5rem",color:"rgba(255,255,255,.4)"}}>1 nəfərlik · Buxar</div>
          </div>
        </div>
        <div style={{fontSize:".58rem",fontWeight:700,color:"rgba(200,169,110,.8)",marginBottom:"6px"}}>⏰ Saat seçin</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"3px",marginBottom:"6px"}}>
          {["15:00","15:30","16:00","16:30","17:00","17:30","18:00","18:30","19:00"].map((t,i)=>(
            <div key={t} style={{padding:"5px 2px",background:i===4?"linear-gradient(135deg,#c8a96e,#e0c07a)":"rgba(255,255,255,.06)",color:i===4?"#0a1520":"rgba(255,255,255,.55)",border:`1px solid ${i===4?"transparent":"rgba(255,255,255,.1)"}`,borderRadius:"6px",fontSize:".52rem",fontWeight:i===4?800:400,textAlign:"center"}}>{t}</div>
          ))}
        </div>
        <div style={{background:"linear-gradient(135deg,#c8a96e,#e0c07a)",color:"#0a1520",borderRadius:"8px",padding:"6px",textAlign:"center",fontSize:".62rem",fontWeight:800}}>Məlumatlar →</div>
      </div>
    </div>,

    // Screen 4: Uğur
    <div key="success" style={{height:"100%",background:"linear-gradient(160deg,#050d17,#0a1f35,#0f2d4a)",display:"flex",flexDirection:"column",alignItems:"center"}}>
      <SB/>
      <div style={{padding:"4px 10px",textAlign:"center",width:"100%"}}>
        <div style={{width:"42px",height:"42px",borderRadius:"50%",background:"rgba(34,197,94,.12)",border:"2px solid rgba(34,197,94,.45)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1rem",margin:"4px auto 6px",color:"#4ade80"}}>✓</div>
        <div style={{fontWeight:800,fontSize:".7rem",color:"#fff",marginBottom:"2px"}}>Rezervasiya Təsdiqləndi!</div>
        <div style={{fontSize:".52rem",color:"rgba(255,255,255,.4)",marginBottom:"8px"}}>Kod: <strong style={{color:"#c8a96e"}}>#BX9K4Z</strong></div>
        <div style={{background:"rgba(255,255,255,.06)",border:"1.5px solid rgba(255,255,255,.12)",borderRadius:"9px",padding:"7px",marginBottom:"6px",display:"inline-block"}}>
          <div style={{width:"55px",height:"55px",margin:"0 auto",background:"repeating-conic-gradient(#c8a96e 0% 25%, #0a1520 0% 50%) 0 0 / 6px 6px",borderRadius:"4px"}}/>
          <div style={{fontSize:".45rem",color:"rgba(255,255,255,.3)",marginTop:"3px",textAlign:"center"}}>Restoranda göstərin</div>
        </div>
        <div style={{background:"rgba(200,169,110,.1)",border:"1px solid rgba(200,169,110,.25)",borderRadius:"7px",padding:"5px 8px",fontSize:".52rem",color:"rgba(200,169,110,.85)",fontWeight:600}}>📱 WhatsApp bildiriş gəldi ✓</div>
      </div>
    </div>,
  ];
  ...
  return (
    <div style={{width:"210px",flexShrink:0}}>
      <style>{`
        @keyframes phoneFloat{0%,100%{transform:translateY(0) rotate(-2deg)}50%{transform:translateY(-14px) rotate(-1.5deg)}}
        @keyframes screenSlideUp{from{opacity:0;transform:translateX(12px)}to{opacity:1;transform:translateX(0)}}
        .phone-screen-anim{animation:screenSlideUp .3s ease}
        @media(max-width:900px){.phone-mockup-wrapper{display:none!important;}.phone-mobile-section{display:flex!important;}}
      `}</style>
      <div style={{
        width:"210px",height:"430px",
        background:"linear-gradient(170deg,#1a2535,#0d1520)",
        borderRadius:"44px",
        padding:"12px 9px 16px",
        boxShadow:"0 50px 120px rgba(0,0,0,.9),0 0 0 1.5px rgba(255,255,255,.1),inset 0 1px 0 rgba(255,255,255,.15)",
        position:"relative",
        animation:"phoneFloat 4s ease-in-out infinite",
      }}>
        {/* Dynamic island */}
        <div style={{position:"absolute",top:"12px",left:"50%",transform:"translateX(-50%)",width:"80px",height:"24px",background:"#000",borderRadius:"20px",zIndex:10,display:"flex",alignItems:"center",justifyContent:"flex-end",padding:"0 8px"}}>
          <div style={{width:"9px",height:"9px",borderRadius:"50%",background:"#111",border:"1.5px solid #1a1a1a"}}/>
        </div>
        {/* Side buttons */}
        <div style={{position:"absolute",right:"-4px",top:"95px",width:"4px",height:"50px",background:"rgba(255,255,255,.08)",borderRadius:"3px"}}/>
        <div style={{position:"absolute",left:"-4px",top:"75px",width:"4px",height:"34px",background:"rgba(255,255,255,.08)",borderRadius:"3px"}}/>
        <div style={{position:"absolute",left:"-4px",top:"118px",width:"4px",height:"34px",background:"rgba(255,255,255,.08)",borderRadius:"3px"}}/>
        <div style={{width:"100%",height:"100%",borderRadius:"34px",overflow:"hidden",position:"relative"}}>
          <div key={screenIdx} className="phone-screen-anim" style={{height:"100%",opacity:fade?1:0,transition:"opacity .25s ease"}}>
            {screens[screenIdx]}
          </div>
        </div>
        <div style={{position:"absolute",bottom:"8px",left:"50%",transform:"translateX(-50%)",width:"55px",height:"4px",background:"rgba(255,255,255,.2)",borderRadius:"2px"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"center",gap:"5px",marginTop:"12px"}}>
        {[0,1,2,3].map(i=>(
          <div key={i} style={{width:i===screenIdx?"20px":"5px",height:"5px",borderRadius:"3px",background:i===screenIdx?ac:"rgba(255,255,255,.2)",transition:"all .3s ease"}}/>
        ))}
      </div>
      <div style={{textAlign:"center",marginTop:"6px",fontSize:".52rem",color:"rgba(255,255,255,.3)",letterSpacing:".1em",textTransform:"uppercase"}}>
        {["Ana Səhifə","Masa Seç","Saat Seç","Təsdiqləndi"][screenIdx]}
      </div>
    </div>
  );
}
...
function HomePage({ navigate, search: searchProp }) {
  ...
  const [activeCat, setActiveCat] = useState("Hamısı");
  const [activeVenue, setActiveVenue] = useState("hamısı");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("reservations").select("id,table_label,time,status").eq("date", today).neq("status","ləğv edildi"),
      ...
      // Boş masa: cəmi masa - bu gün ən az 1 aktiv rezervi olan fərqli masalar
      const bookedTableLabels = new Set((r4.data||[]).map(r=>r.table_label));
      setBookedToday(bookedTableLabels.size);
  ...
  const filtered = useMemo(() => {
    ...
    if (activeVenue !== "hamısı") list = list.filter(r => (r.venue_type||"restoran") === activeVenue);
    ...
    return list;
  }, [restaurants, searchQ, activeCat, activeVenue]);
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .rcard-stripe{height:3px;width:100%;position:absolute;top:0;left:0;z-index:2;}
    .rcard-open-badge{display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:600;padding:3px 9px;border-radius:100px;}
    .rcard-price-badge{font-size:12px;font-weight:700;padding:3px 9px;border-radius:20px;background:#faf5ec;color:#c8a96e;}
    ...
    @keyframes discountPulse{0%,100%{transform:scale(1) rotate(-2deg)}50%{transform:scale(1.06) rotate(2deg)}}
    @keyframes discountShimmer{0%{background-position:-200% center}100%{background-position:200% center}}
    .disc-badge{animation:discountPulse 2.5s ease-in-out infinite;}
  `;
    ...
    <div style={{ fontFamily:"'DM Sans',system-ui,sans-serif", background:"#f5f0e8", minHeight:"100vh", color:"#1a1a2e", overflowX:"hidden" }}>
      ...
      <header style={{ background:"rgba(10,21,32,.75)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", position:"fixed", top:0, left:0, right:0, zIndex:200, height:"58px", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 1.5rem", borderBottom:"1px solid rgba(255,255,255,.07)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:".65rem", cursor:"pointer" }} onClick={() => navigate("/")}>
          <svg width="36" height="36" viewBox="0 0 48 48" fill="none" style={{flexShrink:0}}>
            <rect width="48" height="48" rx="13" fill="#0a1520" opacity=".8"/>
            <rect width="48" height="48" rx="13" stroke="#c8a96e" strokeWidth="1" fill="none" opacity=".3"/>
            <path d="M14 8 L14 22 Q14 27 19 27 L19 40" stroke="#c8a96e" strokeWidth="2.2" fill="none" strokeLinecap="round"/>
            <path d="M14 8 Q14 16 19 16 Q24 16 24 8" stroke="#c8a96e" strokeWidth="2" fill="none" strokeLinecap="round"/>
            <line x1="29" y1="8" x2="29" y2="40" stroke="#c8a96e" strokeWidth="2.2" strokeLinecap="round"/>
            <path d="M25 8 Q25 18 29 20 Q33 18 33 8" stroke="#c8a96e" strokeWidth="2" fill="none" strokeLinecap="round"/>
          ...
          <div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:900, fontSize:"1.05rem", color:"#fff", lineHeight:1.1, letterSpacing:"-.02em" }}><span style={{color:"#c8a96e"}}>1</span>rezerv</div>
            <div style={{ fontSize:".42rem", letterSpacing:".2em", textTransform:"uppercase", color:"rgba(200,169,110,.4)", marginTop:"1px" }}>Rezervasiya Platforması</div>
          </div>
        </div>
      ...
      <div className={heroVisible ? "hero-in" : ""}
        ...
        {/* Phone Demo Mockup */}
        <div className="phone-mockup-wrapper" style={{ position:"absolute", right:"6%", top:"50%", transform:"translateY(-50%)", zIndex:5 }}>
          <PhoneDemoMockup />
        </div>
      </div>


      {/* ── MOBILE PHONE DEMO (yalnız mobile) ── */}
      <div className="phone-mobile-section" style={{
        display:"none",
        background:"linear-gradient(180deg,#0a1f35 0%,#0f2a42 100%)",
        ...
        {/* Arxa fon orb-lar */}
        <div style={{position:"absolute",top:"-40px",right:"-40px",width:"200px",height:"200px",borderRadius:"50%",background:"radial-gradient(circle,rgba(200,169,110,.12) 0%,transparent 70%)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",bottom:"-20px",left:"-30px",width:"160px",height:"160px",borderRadius:"50%",background:"radial-gradient(circle,rgba(18,72,122,.25) 0%,transparent 70%)",pointerEvents:"none"}}/>

        {/* Başlıq */}
        <div style={{textAlign:"center",zIndex:1}}>
          <div style={{fontSize:".72rem",fontWeight:700,color:"rgba(200,169,110,.9)",letterSpacing:".12em",textTransform:"uppercase",marginBottom:".4rem"}}>Necə işləyir?</div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:"1.35rem",fontWeight:800,color:"#fff",lineHeight:1.25}}>3 addımda rezervasiya</div>
        </div>

        {/* Phone mockup — mərkəzdə, böyük */}
        <div style={{zIndex:1,transform:"scale(1.25)",transformOrigin:"top center",marginTop:"1rem",marginBottom:"1.5rem"}}>
          <PhoneDemoMockup />
        </div>

        {/* Addım labels */}
        <div style={{display:"flex",gap:"1rem",zIndex:1,marginTop:".5rem",flexWrap:"wrap",justifyContent:"center"}}>
          {[["🏛️","Restoran seç"],["🪑","Masa seç"],["✅","Təsdiqləndi"]].map(([icon,label],i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:".4rem",background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.1)",borderRadius:"100px",padding:".35rem .85rem",fontSize:".75rem",color:"rgba(255,255,255,.8)",fontWeight:600}}>
      ...
      <div style={{ maxWidth:"1300px", margin:"0 auto", padding:"2.5rem 2rem 5rem" }}>
        ...
        {/* ── VENUE TYPE TABS ── */}
        <div style={{ display:"flex", gap:".75rem", marginBottom:"1.5rem", overflowX:"auto", paddingBottom:".2rem" }}>
          {[
            { key:"hamısı",  icon:"✨", label:"Hamısı"   },
            { key:"restoran",icon:"🍽️", label:"Restoran" },
            { key:"lounge",  icon:"🛋️", label:"Lounge"   },
            { key:"pub",     icon:"🍺", label:"Pub"      },
          ].map(v => {
            const cnt = v.key==="hamısı" ? restaurants.length : restaurants.filter(r=>(r.venue_type||"restoran")===v.key).length;
            const active = activeVenue===v.key;
            return (
              <button key={v.key} onClick={() => { setActiveVenue(v.key); setActiveCat("Hamısı"); }}
                style={{
                  display:"flex", alignItems:"center", gap:".5rem",
                  padding:".6rem 1.25rem",
                  background: active ? "#0a1f35" : "#fff",
                  color: active ? "#fff" : "#555",
                  border: `2px solid ${active ? "#c8a96e" : "#e2e0da"}`,
                  borderRadius:"100px", fontSize:".85rem", fontWeight:active?700:500,
                  cursor:"pointer", whiteSpace:"nowrap", transition:"all .18s",
                  fontFamily:"'DM Sans',sans-serif",
                  boxShadow: active ? "0 4px 14px rgba(10,21,35,.2)" : "none",
                }}>
                <span style={{fontSize:"1rem"}}>{v.icon}</span>
                {v.label}
                <span style={{
                  background: active ? "rgba(200,169,110,.3)" : "#f1f0ec",
                  color: active ? "#e8c96e" : "#888",
                  borderRadius:"100px", padding:".1rem .5rem",
                  fontSize:".72rem", fontWeight:700, minWidth:"20px", textAlign:"center"
                }}>{cnt}</span>
              </button>
            );
          })}
        ...
        {/* ── CUISINE CHIPS (venue seçildikdən sonra) ── */}
        <div style={{ display:"flex", gap:".55rem", flexWrap:"wrap", marginBottom:"2rem", overflowX:"auto", paddingBottom:".2rem" }}>
          {allCuisines.map(cat => (
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:"1.8rem" }}>
          <div>
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.9rem", fontWeight:700, color:"#1a1a2e" }}>
              {{"hamısı":"Bütün Yerlər","restoran":"Restoranlar","lounge":"Lounge-lar","pub":"Pub-lar"}[activeVenue]}
            </h2>
            <div style={{ fontSize:".8rem", color:"#9c9c9c", marginTop:".25rem" }}>{filtered.length} yer tapıldı</div>
          </div>
        ...
        {!loading && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:"1.5rem" }}>
            {filtered.map(r => {
              ...
              const ac = r.accent || "#c8a96e";
              // İş saatlarından açıq/bağlı
              const nowHm = new Date().getHours()*60+new Date().getMinutes();
              let isOpenNow = null;
              if(r.hours){
                const hm = r.hours.replace(/\s/g,"").match(/(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})/);
                if(hm){ const o=parseInt(hm[1])*60+parseInt(hm[2]); const c=parseInt(hm[3])*60+parseInt(hm[4]); isOpenNow=c<=o?(nowHm>=o||nowHm<c):(nowHm>=o&&nowHm<c); }
              }
              return (
                <div
                  key={r.id}
                ...
                >
                  {/* Accent stripe */}
                  <div className="rcard-stripe" style={{ background:`linear-gradient(90deg,${ac},${ac}99)` }}/>

                  {/* Discount badge */}
                  {r.discount_percent > 0 && (
                    <div className="disc-badge" style={{ position:"absolute", top:"8px", left:"8px", zIndex:3, background:"linear-gradient(135deg,#dc2626,#ef4444)", color:"#fff", borderRadius:"10px", padding:".3rem .7rem", fontSize:".72rem", fontWeight:900, letterSpacing:".02em", boxShadow:"0 4px 12px rgba(220,38,38,.45)" }}>
                      🔥 -{r.discount_percent}%
                    </div>
                  )}

                  {/* Image area */}
                  <div style={{ position:"relative", height:"180px", overflow:"hidden", background: r.cover_url ? "transparent" : `linear-gradient(135deg,${ac}cc,${ac}55)` }}>
                      ...
                      {r.cover_url
                        ...
                        : (
                          <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center" }}>
                            <span style={{ fontFamily:"'Playfair Display',serif", fontSize:"4rem", fontWeight:900, color:"rgba(255,255,255,.12)", letterSpacing:"-.05em", userSelect:"none" }}>{initials(r.name)}</span>
                          </div>
                        )
                      }
                    </div>
                    {/* Gradient overlay */}
                    <div style={{ position:"absolute", inset:0, background:"linear-gradient(to bottom,transparent 25%,rgba(0,0,0,.68))" }}/>

                    {/* Discount badge — top right, below rating */}
                    {r.discount_percent > 0 && (
                      <div className="disc-badge" style={{ position:"absolute", top:"10px", right:"10px", zIndex:3, background:"linear-gradient(135deg,#dc2626,#ef4444)", color:"#fff", borderRadius:"10px", padding:".28rem .65rem", fontSize:".68rem", fontWeight:900, letterSpacing:".02em", boxShadow:"0 4px 12px rgba(220,38,38,.45)" }}>
                        🔥 -{r.discount_percent}%
                      </div>
                    )}
                    ...
                    <div style={{ position:"absolute", top:"10px", left:"10px" }}>
                      {isOpenNow!==null && (
                        <span className="rcard-open-badge" style={{ background:isOpenNow?"rgba(34,197,94,.18)":"rgba(239,68,68,.15)", color:isOpenNow?"#4ade80":"#f87171", border:`1px solid ${isOpenNow?"rgba(34,197,94,.3)":"rgba(239,68,68,.25)"}` }}>
                          <span style={{ width:"6px", height:"6px", borderRadius:"50%", background:isOpenNow?"#22c55e":"#ef4444", display:"inline-block" }}/>
                          {isOpenNow?"Açıqdır":"Bağlıdır"}
                        </span>
                      )}
                      {!isOpenNow && !r.available && (
                        <span className="rcard-open-badge" style={{ background:"rgba(239,68,68,.15)", color:"#f87171", border:"1px solid rgba(239,68,68,.25)" }}>
                    ...
                    {/* Rating — top right, below discount if discount exists */}
                    {rating && !r.discount_percent && (
                      <div style={{ position:"absolute", top:"10px", right:"10px", background:"rgba(255,255,255,.92)", borderRadius:"100px", padding:".25rem .7rem", display:"flex", alignItems:"center", gap:".3rem", fontSize:".75rem", fontWeight:700, color:"#1a1a2e", boxShadow:"0 2px 8px rgba(0,0,0,.15)" }}>
                        <span style={{ color:"#c8a96e" }}>★</span>{rating}
                      </div>
                    )}
                    {rating && r.discount_percent > 0 && (
                      <div style={{ position:"absolute", top:"38px", right:"10px", background:"rgba(255,255,255,.92)", borderRadius:"100px", padding:".25rem .7rem", display:"flex", alignItems:"center", gap:".3rem", fontSize:".75rem", fontWeight:700, color:"#1a1a2e", boxShadow:"0 2px 8px rgba(0,0,0,.15)" }}>
                        <span style={{ color:"#c8a96e" }}>★</span>{rating}
                      </div>
                    )}
                    ...
                    <div style={{ position:"absolute", bottom:"10px", left:"12px", right:"12px" }}>
                      <div style={{ fontSize:".6rem", letterSpacing:".12em", textTransform:"uppercase", color:"rgba(255,255,255,.5)", fontWeight:500, marginBottom:"2px" }}>{normCuisine(r.cuisine)}</div>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.25rem", fontWeight:700, color:"#fff", lineHeight:1.1 }}>{r.name}</div>
                    </div>
                  </div>

                  {/* Card body */}
                  <div style={{ padding:"1rem 1.1rem 1.1rem" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:".35rem", fontSize:".75rem", color:"#888", marginBottom:".7rem", flexWrap:"wrap" }}>
                      <span>📍</span><span>{r.location}</span>
                      {r.hours && <><span style={{ color:"#d0ccc6" }}>·</span><span>{r.hours}</span></>}
                    </div>
                    {r.tags.length > 0 && (
                      <div style={{ display:"flex", flexWrap:"wrap", gap:".3rem", marginBottom:".8rem" }}>
                        {r.tags.slice(0,3).map(t => (
                          <span key={t} style={{ fontSize:".62rem", padding:".18rem .55rem", borderRadius:"100px", background:"#f0ece4", color:"#666", fontWeight:500 }}>{t}</span>
                        ))}
                      </div>
                    )}
                    <div style={{ display:"flex", alignItems:"center", gap:".6rem" }}>
                      <span className="rcard-price-badge">{r.priceRange||"₼₼"}</span>
                      {r.available
                        ? <button style={{ flex:1, background:"#1a1a2e", color:"#fff", border:"none", borderRadius:"10px", padding:".72rem", fontSize:".85rem", fontWeight:600, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:".4rem", transition:"background .2s" }}
                            onMouseEnter={e=>{e.currentTarget.style.background="#12487a";}}
                            onMouseLeave={e=>{e.currentTarget.style.background="#1a1a2e";}}>
                            Rezervasiya et →
                          </button>
                        : <div style={{ flex:1, background:"#f0ece4", color:"#aaa", borderRadius:"10px", padding:".72rem", fontSize:".82rem", textAlign:"center", fontWeight:500 }}>Bu gün yer yoxdur</div>
                      }
                    </div>
                  </div>
                </div>
...
function getTimeSlots(hours) {
  let startH = 12, endH = 22;
  if (hours) {
    const m = hours.replace(/\s/g,"").match(/(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})/);
    if (m) {
      startH = parseInt(m[1]);
      let eH = parseInt(m[3]);
      const eM = parseInt(m[4]);
      // Gecə yarısını keçən saat (məs: 15:00-02:00)
      if (eH < startH) eH += 24;
      // Son slot bağlanma saatından 1.5 saat əvvəl (90 dəq reservation duration)
      const closeTotal = eH * 60 + eM;
      const lastSlotTotal = closeTotal - 90;
      endH = Math.floor(lastSlotTotal / 60) % 24;
      const endM = lastSlotTotal % 60;
      if (endM === 0) endH = endH; else endH = endH; // already correct
      // Simplified: just use eH-2 as last hour
      endH = (eH - 2 + 24) % 24;
      if (endH < startH && eH < 24) endH = 23;
    }
  }
  const slots = [];
  let h = startH;
  let safetyCount = 0;
  while (h !== (endH + 1) % 24 && safetyCount < 48) {
    const hh = h % 24;
    slots.push(String(hh).padStart(2,"0") + ":00");
    const hh30 = hh;
    const next30 = (hh * 60 + 30);
    const next30H = Math.floor(next30 / 60) % 24;
    if (next30H !== (endH + 1) % 24) {
      slots.push(String(hh).padStart(2,"0") + ":30");
    }
    h = (h + 1) % 24;
    safetyCount++;
    if (h === 0 && startH > 0) break; // safety for overnight
  }
  return slots;
}
...
// DURATION: default 90 dəq. Admin panel Profil bölməsindən dəyişdirilə bilər.
function getResDuration(rest, selDur) {
  if (selDur && selDur > 0) return selDur;
  if (rest?.res_duration) return parseInt(rest.res_duration);
  return parseInt(localStorage.getItem("res_duration") || "90");
}

function getTableFreeSlots(tableLabel, allRevs, date, restHours, rest, selDur) {
  const DURATION = getResDuration(rest, selDur);
  const slots = restHours ? getTimeSlots(restHours) : ALL_TIME_SLOTS;
  ...
  return slots.filter(slot => {
    const s = timeToMins(slot);
    // Slot bloklanır yalnız əgər: mövcud rezervasiya bu slotun İÇİNDƏ başlayır
    // və ya bu slot mövcud rezervasiyanın İÇİNDƏ qalır
    // Geri bloklama yoxdur: 19:00 slotu 20:00 rezervasiyası üçün bloklanmır
    return !occupied.some(r => {
      // Bu slot mövcud rezervasiyanın içindədirmi?
      return s >= r.s && s < r.e;
    });
  });
}
...
function getTableAllSlots(tableLabel, allRevs, date, restHours, rest, selDur) {
  const DURATION = getResDuration(rest, selDur);
  const slots = restHours ? getTimeSlots(restHours) : ALL_TIME_SLOTS;
  const occupied = allRevs
    .filter(r => r.table_label===tableLabel && r.date===date && r.status!=="ləğv edildi")
  ...
  return slots.map(slot => {
    const s = timeToMins(slot);
    const isBooked = occupied.some(r => s >= r.s && s < r.e);
...
function MainApp({ navigate, restSlug, initialStep, preTable }) {
  ...
  const [dateAlertPulse, setDateAlertPulse] = useState(false);
  const timeSlotRef = useRef(null);
  const davametRef = useRef(null);
  const datePickerRef = useRef(null);
  const [cart,setCart]           = useState({});
  const [guests,setGuests]       = useState(2);
  const [selDuration,setSelDuration] = useState(0); // 0 = restoran default
  ...
  // localStorage-da noemre melumati: {count, blockedUntil}
  const getPhoneData = (phone) => {
    try { return JSON.parse(localStorage.getItem("otp_"+phone)||"{}"); }
    catch(e) { return {}; }
  };
  const setPhoneData = (phone, data) => {
    localStorage.setItem("otp_"+phone, JSON.stringify(data));
  };
  const isPhoneBlocked = (phone) => {
    const d = getPhoneData(phone);
    if(!d.blockedUntil) return false;
    if(Date.now() < d.blockedUntil) return true;
    setPhoneData(phone, {});
    return false;
  };
  const getBlockMinutes = (phone) => {
    const d = getPhoneData(phone);
    if(!d.blockedUntil) return 0;
    return Math.ceil((d.blockedUntil - Date.now()) / 60000);
  };
  const getAttemptCount = (phone) => getPhoneData(phone).count || 0;
  ...
  const reset=()=>{navigate("/");setSelRest(null);setSelDate("");setSelTime("");setSelTbl(null);setHover(null);setCart({});setGuests(2);setSelDuration(0);setCustName("");setCustPhone("");setSpecialOccasion("");setBkCode("");setSaveErr("");setBkd([]);setSlots({});setAllRevs([]);};

  // Final rezervasiya — OTP keçdikdən sonra çağrılır
  const submitReservationFinal = async () => {
    ...
    const isAutoConfirm = selRest?.auto_confirm === true;
    const DEFAULT_BOOKING_TPL = isAutoConfirm
      ? `🍽️ *{restaurant}* — Rezervasiya Təsdiqləndi!\n\nSalam, {name}!\n📅 Tarix: {date}\n⏰ Saat: {time}\n👥 Nəfər: {guests}\n🪑 Masa: {table}\n🔖 Kod: *{code}*\n\nSizi gözləyirik! 🍽️`
      : `🍽️ *{restaurant}* — Rezervasiya Alındı!\n\nSalam, {name}!\n📅 Tarix: {date}\n⏰ Saat: {time}\n👥 Nəfər: {guests}\n🪑 Masa: {table}\n🔖 Kod: *{code}*\n\nTəsdiq üçün gözləyin. ✅`;
    const tpl = (isAutoConfirm ? null : localStorage.getItem("wa_tpl_booking")) || DEFAULT_BOOKING_TPL;
    let waMsg = tpl
      .replace(/{name}/g, custName).replace(/{restaurant}/g, selRest.name)
      .replace(/{date}/g, selDate).replace(/{time}/g, selTime)
      .replace(/{guests}/g, guests).replace(/{table}/g, selTable.label).replace(/{code}/g, code);
    if(cartTotal>0) waMsg += "\n💰 Ön sifariş: "+cartTotal+"₼";
    if(specialOccasion) waMsg += "\n🎉 Xüsusi gün: "+specialOccasion;
    sendWA(custPhone, waMsg);
  };

  const submitReservation = async () => {
    // 1. Blok yoxlama — əvvəlki OTP-ni rədd etmişsə 1 saat gözləsin
    if(isPhoneBlocked(custPhone)) {
      const mins = getBlockMinutes(custPhone);
      setSaveErr(`⛔ Təsdiq kodu rədd edilib. ${mins} dəqiqə sonra yenidən cəhd edin.`);
      return;
    }
    const d = getPhoneData(custPhone);
    const attempts = d.count || 0;
    // 2. 2-ci+ cəhd — OTP lazımdır
    if(attempts >= 1) {
      setSaving(true); setSaveErr("");
      const code = generateOTP();
      setOtpCode(code);
      const msg = `🔐 *1rezerv Təsdiq Kodu*

Kodunuz: *${code}*

Bu kodu heç kimə verməyin.`;
      await sendWA(custPhone, msg);
      setSaving(false);
      setOtpStep(true);
      return;
    }
    // 3. 1-ci rezervasiya — birbaşa keçir, sayı artır
    setPhoneData(custPhone, { count: 1 });
    await submitReservationFinal();
  };

  const verifyOTP = () => {
    if(otpInput.trim() === otpCode) {
      // Duzgun kod — sayı artır, rezervasiyanı tamamla
      const d = getPhoneData(custPhone);
      setPhoneData(custPhone, { count: (d.count||0) + 1 });
      setOtpStep(false);
      setOtpInput(""); setOtpError("");
      submitReservationFinal();
    } else {
      // Yanlış kod — 1 saat blokla
      setPhoneData(custPhone, {
        count: getPhoneData(custPhone).count || 1,
        blockedUntil: Date.now() + 60 * 60 * 1000  // 1 saat
      });
      setOtpStep(false);
      setOtpInput("");
      setSaveErr("⛔ Yanlış kod. Bu nömrə 1 saat müddətinə bloklandı.");
    }
  };
  ...
  const css=`
    ...
    *{box-sizing:border-box;margin:0;padding:0;}body{background:linear-gradient(155deg,#050d17 0%,#0a1f35 45%,#0f2d4a 100%) fixed;}
    input,select,textarea{font-family:'Inter',system-ui,sans-serif;}
    .card{transition:box-shadow .2s,transform .2s;}.card:hover{box-shadow:0 8px 32px rgba(0,0,0,.3);transform:translateY(-3px);}
    ...
    ::-webkit-scrollbar{width:5px;}::-webkit-scrollbar-thumb{background:rgba(200,169,110,.3);border-radius:10px;}
    .nx-card-free{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:1rem .85rem;text-align:center;cursor:pointer;transition:all .2s;}
    .nx-card-free:hover{background:rgba(255,255,255,.08);transform:translateY(-2px);}
    .nx-card-sel{background:rgba(200,169,110,.1);border:1.5px solid rgba(200,169,110,.5);border-radius:14px;padding:1rem .85rem;text-align:center;cursor:pointer;transition:all .2s;}
    .nx-card-booked{background:rgba(239,68,68,.05);border:1px solid rgba(239,68,68,.18);border-radius:14px;padding:1rem .85rem;text-align:center;opacity:.6;}
    .nx-slot-free{padding:.5rem .4rem;background:rgba(255,255,255,.06);color:rgba(255,255,255,.75);border:1px solid rgba(255,255,255,.1);border-radius:8px;font-size:.78rem;font-weight:500;cursor:pointer;font-family:inherit;text-align:center;transition:all .15s;}
    .nx-slot-free:hover{background:rgba(200,169,110,.15);border-color:rgba(200,169,110,.35);color:#c8a96e;}
    .nx-slot-sel{padding:.5rem .4rem;background:rgba(200,169,110,.2);color:#c8a96e;border:1.5px solid rgba(200,169,110,.6);border-radius:8px;font-size:.78rem;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;}
    .nx-slot-booked{padding:.5rem .4rem;background:rgba(239,68,68,.06);color:rgba(239,68,68,.5);border:1px solid rgba(239,68,68,.15);border-radius:8px;font-size:.75rem;font-weight:500;cursor:not-allowed;text-decoration:line-through;font-family:inherit;text-align:center;position:relative;}
    @keyframes datePulse{0%,100%{box-shadow:0 0 0 0 rgba(200,169,110,.7)}50%{box-shadow:0 0 0 8px rgba(200,169,110,.0)}}
  `;
  const inp={width:"100%",border:"none",background:"rgba(255,255,255,.07)",borderRadius:"8px",padding:".65rem .9rem",fontSize:".9rem",color:"#fff",outline:"none",fontFamily:"'Inter',system-ui,sans-serif"};
  const box={background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:"12px",padding:".9rem 1rem"};
  const lbl={fontSize:".65rem",fontWeight:600,letterSpacing:".1em",textTransform:"uppercase",color:"rgba(255,255,255,.4)",marginBottom:".4rem",display:"block"};
  const back={background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"rgba(255,255,255,.6)",padding:".5rem 1rem",fontFamily:"inherit",fontSize:".8rem",cursor:"pointer",borderRadius:"8px",marginBottom:"1.5rem",fontWeight:500};

  return (
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"linear-gradient(155deg,#050d17 0%,#0a1f35 45%,#0f2d4a 100%)",minHeight:"100vh",color:"#fff"}}>
      <style>{css}</style>
      <header style={{background:"rgba(5,13,23,.85)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,.07)",position:"sticky",top:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 2rem",height:"60px",boxShadow:"0 1px 20px rgba(0,0,0,.3)"}}>
        ...
        <div><div style={{fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:"1.05rem",letterSpacing:"-.02em",color:"#fff"}}><span style={{color:"#c8a96e"}}>1</span>rezerv</div><div style={{fontSize:".42rem",letterSpacing:".2em",textTransform:"uppercase",color:"rgba(200,169,110,.4)",marginTop:"1px"}}>Rezervasiya Platforması</div></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:".8rem"}}>
      ...
      {!["home","success","cancel"].includes(step)&&(
        <div style={{background:"rgba(255,255,255,.04)",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",justifyContent:"center",overflow:"hidden"}}>
          {["Masa Seçimi","Menyu","Məlumatlar"].map((s,i)=>{
            const done=stepNum>i+1,active=stepNum===i+1;
            return (
              <div key={s} style={{display:"flex",alignItems:"center",gap:".3rem",padding:".75rem .8rem",borderBottom:`2px solid ${done?"#c8a96e":active?"rgba(200,169,110,.5)":"transparent"}`,flex:1,justifyContent:"center",minWidth:0}}>
                <div style={{width:"20px",height:"20px",borderRadius:"50%",background:done?"rgba(200,169,110,.2)":active?"rgba(200,169,110,.1)":"rgba(255,255,255,.05)",border:`1.5px solid ${done?"#c8a96e":active?"rgba(200,169,110,.5)":"rgba(255,255,255,.15)"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:".6rem",fontWeight:700,color:done?"#c8a96e":active?"#c8a96e":"rgba(255,255,255,.3)",flexShrink:0}}>{done?"✓":i+1}</div>
                <span style={{fontSize:".65rem",fontWeight:600,letterSpacing:".03em",textTransform:"uppercase",color:done?"#c8a96e":active?"rgba(255,255,255,.85)":"rgba(255,255,255,.3)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{s}</span>
              </div>
            );
          })}
        </div>
      ...
      {step==="datetime"&&selRest&&(
        <div style={{maxWidth:"720px",margin:"0 auto",padding:".8rem"}}>
          ...
          <div style={{...box,marginBottom:"1.2rem",display:"flex",gap:"1rem",alignItems:"center",background:"rgba(200,169,110,.08)",border:"1.5px solid rgba(200,169,110,.25)"}}>
            <div style={{fontSize:"1.8rem"}}>🪑</div>
            <div style={{flex:1}}>
              <div style={{fontWeight:700,fontSize:".95rem",color:"#c8a96e"}}>Masa {selTable?.label} seçildi</div>
              <div style={{fontSize:".75rem",color:"rgba(200,169,110,.55)"}}>{selRest.name} · {selDate} · ⏰ {selTime} · {selTable?.seats} nəfərlik</div>
            </div>
            <button onClick={()=>navigateStep("table")} style={{background:"none",border:"1px solid rgba(200,169,110,.25)",borderRadius:"6px",padding:".3rem .7rem",fontSize:".72rem",color:"#c8a96e",cursor:"pointer",fontFamily:"inherit"}}>Dəyiş</button>
          </div>

          <div style={{fontWeight:800,fontSize:"1.3rem",letterSpacing:"-.02em",marginBottom:".3rem",color:"#fff"}}>Məlumatlarınız</div>
          <div style={{fontSize:".78rem",color:"rgba(255,255,255,.35)",marginBottom:"1.2rem"}}>Rezervasiyanı tamamlamaq üçün doldurun</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"1rem",marginBottom:"1rem"}}>
            ...
            <div style={box}>
              ...
              {custPhone.length > 4 && !["50","51","55","60","70","77","99"].includes(custPhone.slice(4,6)) && (
                <div style={{fontSize:".72rem",color:"#f87171",marginTop:".3rem"}}>⚠️ Düzgün operator prefiksi daxil edin</div>
              )}
              {custPhone.length > 4 && ["50","51","55","60","70","77","99"].includes(custPhone.slice(4,6)) && custPhone.slice(4).length === 9 && (
                <div style={{fontSize:".72rem",color:"#4ade80",marginTop:".3rem"}}>✓ Nömrə düzgündür</div>
              )}
            </div>
          </div>
          ...
          {/* Oturma müddəti */}
          {(() => {
            const maxDur = selRest?.res_duration || 120;
            const opts = [60,90,120,150,180].filter(v => v <= maxDur);
            if(opts.length <= 1) return null;
            return (
              <div style={{...box,marginBottom:"1.5rem"}}>
                <label style={lbl}>⏱️ Oturma müddəti <span style={{fontWeight:400,color:"rgba(200,169,110,.4)"}}>(istəyə görə)</span></label>
                <div style={{display:"flex",gap:".5rem",flexWrap:"wrap",marginTop:".5rem"}}>
                  <button onClick={()=>setSelDuration(0)}
                    style={{padding:".4rem .9rem",borderRadius:"8px",border:selDuration===0?"1.5px solid #c8a96e":"1px solid rgba(255,255,255,.1)",background:selDuration===0?"rgba(200,169,110,.15)":"transparent",color:selDuration===0?"#c8a96e":"rgba(255,255,255,.5)",fontSize:".82rem",cursor:"pointer",fontFamily:"inherit"}}>
                    🔄 Standart ({maxDur} dəq)
                  </button>
                  {opts.filter(v=>v!==maxDur).map(v=>(
                    <button key={v} onClick={()=>setSelDuration(v)}
                      style={{padding:".4rem .9rem",borderRadius:"8px",border:selDuration===v?"1.5px solid #c8a96e":"1px solid rgba(255,255,255,.1)",background:selDuration===v?"rgba(200,169,110,.15)":"transparent",color:selDuration===v?"#c8a96e":"rgba(255,255,255,.5)",fontSize:".82rem",cursor:"pointer",fontFamily:"inherit"}}>
                      {v} dəq
                    </button>
                  ))}
                </div>
                {selDuration>0&&<div style={{marginTop:".5rem",fontSize:".75rem",color:"rgba(200,169,110,.6)"}}>✓ {selDuration} dəqiqəlik masa seçdiniz</div>}
              </div>
            );
          })()}
          <div style={{...box,marginBottom:"1.5rem",border:"1px solid rgba(200,169,110,.2)",background:"rgba(200,169,110,.06)"}}>
            <label style={{...lbl,color:"rgba(200,169,110,.6)"}}>🎉 Xüsusi gün <span style={{fontWeight:400,color:"rgba(200,169,110,.4)"}}>(istəyə görə)</span></label>
            <select value={specialOccasion} onChange={e=>setSpecialOccasion(e.target.value)} style={{...inp,cursor:"pointer",background:"transparent",color:"rgba(200,169,110,.8)"}}>
            ...
            {specialOccasion&&<div style={{marginTop:".5rem",fontSize:".78rem",color:"#c8a96e",fontWeight:600}}>✨ Restoran sizin üçün xüsusi hazırlıq edəcək!</div>}
          </div>

          {saveError&&<div style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",borderRadius:"8px",padding:".75rem 1rem",marginBottom:"1rem",color:"#f87171",fontSize:".85rem"}}>{saveError}</div>}
          ...
          <button className="btnp" onClick={submitReservation} disabled={!canSubmit}
            style={{background:canSubmit?"linear-gradient(135deg,#c8a96e,#e0c07a)":"rgba(255,255,255,.06)",color:canSubmit?"#0a1520":"rgba(255,255,255,.25)",border:"none",padding:".9rem 2rem",fontFamily:"inherit",fontSize:".95rem",fontWeight:700,cursor:canSubmit?"pointer":"not-allowed",borderRadius:"10px"}}>
        ...
        <div style={{maxWidth:"960px",margin:"0 auto",padding:".8rem"}} onClick={()=>calendarOpen&&setCalOpen(false)}>
          ...
          <div style={{...box,marginBottom:"1rem",padding:".8rem 1rem"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
              <div style={{width:"40px",height:"40px",borderRadius:"10px",background:"rgba(200,169,110,.1)",border:"1px solid rgba(200,169,110,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.3rem",flexShrink:0}}>{selRest.emoji||"🍽️"}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:800,fontSize:".95rem",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",color:"#fff"}}>{selRest.name}</div>
                <div style={{fontSize:".68rem",color:"rgba(255,255,255,.35)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{selRest.cuisine} · {selRest.location}</div>
              </div>
              {/* Tarix düyməsi — sağda */}
              <div style={{position:"relative",flexShrink:0}} ref={datePickerRef}>
                <button onClick={()=>setCalOpen(p=>!p)}
                  style={{display:"flex",alignItems:"center",gap:".4rem",background:selDate?"rgba(200,169,110,.15)":dateAlertPulse?"rgba(200,169,110,.25)":"rgba(255,255,255,.06)",border:`1px solid ${selDate?"rgba(200,169,110,.4)":dateAlertPulse?"rgba(200,169,110,.8)":"rgba(255,255,255,.1)"}`,borderRadius:"10px",padding:".5rem .75rem",cursor:"pointer",fontFamily:"inherit",fontSize:".78rem",fontWeight:600,color:selDate?"#c8a96e":dateAlertPulse?"#c8a96e":"rgba(255,255,255,.5)",whiteSpace:"nowrap",transition:"all .3s",animation:dateAlertPulse?"datePulse .5s ease infinite":"none"}}>
                  📅 {selDate ? new Date(selDate+"T12:00:00").toLocaleDateString("az-AZ",{day:"numeric",month:"short"}) : "Tarix seçin"}
                  <span style={{fontSize:".6rem",color:"rgba(255,255,255,.3)"}}>{calendarOpen?"▲":"▼"}</span>
                </button>
                {dateAlertPulse && !selDate && (
                  <div style={{position:"absolute",top:"calc(100% + 6px)",right:0,background:"rgba(200,169,110,.95)",color:"#0a1520",borderRadius:"8px",padding:".4rem .85rem",fontSize:".72rem",fontWeight:700,whiteSpace:"nowrap",zIndex:50,boxShadow:"0 4px 16px rgba(200,169,110,.4)"}}>
                    ⬆️ Əvvəlcə tarix seçin
                  </div>
                )}
                {calendarOpen&&(
                  <div style={{position:"absolute",top:"calc(100% + 8px)",right:0,zIndex:999}}
                    ...
                    <CalendarPicker value={selDate} minDate={today}
                      onChange={d=>{
                        ...
                        setCalOpen(false);
                        setSelTime("");
                        // Tarix seçildikdə time slot-a scroll et
                        setTimeout(()=>timeSlotRef.current?.scrollIntoView({behavior:"smooth",block:"start"}), 300);
                      }}/>
                  </div>
                )}
              </div>
          ...
          {dbTables.length===0?(
            <div style={{background:"rgba(255,255,255,.04)",borderRadius:"14px",border:"1px solid rgba(255,255,255,.08)",padding:"3rem",textAlign:"center",color:"rgba(255,255,255,.4)"}}>
            ...
            <>
              {/* Discount banner */}
              {selRest.discount_percent > 0 && (
                <div style={{background:"linear-gradient(135deg,rgba(220,38,38,.15),rgba(239,68,68,.08))",border:"1.5px solid rgba(220,38,38,.3)",borderRadius:"12px",padding:".85rem 1.1rem",marginBottom:"1rem",display:"flex",alignItems:"center",gap:".8rem"}}>
                  <div className="disc-badge" style={{fontSize:"1.6rem",flexShrink:0}}>🔥</div>
                  <div>
                    <div style={{fontWeight:800,fontSize:".95rem",color:"#f87171",letterSpacing:"-.01em"}}>-{selRest.discount_percent}% ENDİRİM AKTİVDİR!</div>
                    <div style={{fontSize:".72rem",color:"rgba(255,255,255,.45)",marginTop:"1px"}}>Bütün menyu maddələrinə tətbiq edilir</div>
                  </div>
                </div>
              )}
              {/* Legenda */}
              <div style={{display:"flex",gap:"1rem",marginBottom:"1rem",flexWrap:"wrap",alignItems:"center"}}>
                {[
                  {bg:"rgba(255,255,255,.05)",bc:"rgba(255,255,255,.12)",tc:"rgba(255,255,255,.5)",l:"Boş"},
                  {bg:"rgba(200,169,110,.12)",bc:"rgba(200,169,110,.45)",tc:"#c8a96e",l:"Seçilmiş"},
                  {bg:"rgba(239,68,68,.06)",bc:"rgba(239,68,68,.2)",tc:"#f87171",l:"Tam dolu"}
                ].map(s=>(
                  <span key={s.l} style={{fontSize:".72rem",color:s.tc,fontWeight:600,background:s.bg,border:`1px solid ${s.bc}`,borderRadius:"20px",padding:".2rem .7rem"}}>{s.l}</span>
                ))}
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:".75rem",marginBottom:"1.2rem"}}>
                {dbTables.map(t=>{
                  const freeSlots = selDate ? getTableFreeSlots(t.label, allRevs, selDate, selRest?.hours, selRest, selDuration) : getTimeSlots(selRest?.hours);
                  const isFullyBooked = selDate && freeSlots.length===0;
                  const isSelected=selTable?.id===t.id;
                  const svgColor = isSelected?"#c8a96e":isFullyBooked?"rgba(239,68,68,.4)":"rgba(255,255,255,.3)";
                  const svgColorChair = isSelected?"rgba(200,169,110,.7)":isFullyBooked?"rgba(239,68,68,.25)":"rgba(255,255,255,.18)";
                  const tableShape = t.shape==="round" ? (
                    <svg viewBox="0 0 44 44" width="38" height="38" style={{display:"block",margin:"0 auto"}}>
                      <circle cx="22" cy="24" r="13" fill="none" stroke={svgColor} strokeWidth="1.8"/>
                      <rect x="16" y="7" width="12" height="7" rx="3" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                      <rect x="16" y="35" width="12" height="7" rx="3" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                    </svg>
                  ) : t.shape==="rect6" ? (
                    <svg viewBox="0 0 48 44" width="42" height="38" style={{display:"block",margin:"0 auto"}}>
                      <rect x="4" y="13" width="40" height="20" rx="4" fill="none" stroke={svgColor} strokeWidth="1.8"/>
                      <rect x="5" y="7" width="8" height="7" rx="2" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                      <rect x="20" y="7" width="8" height="7" rx="2" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                      <rect x="35" y="7" width="8" height="7" rx="2" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                      <rect x="5" y="32" width="8" height="7" rx="2" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                      <rect x="35" y="32" width="8" height="7" rx="2" fill="none" stroke={svgColorChair} strokeWidth="1.3"/>
                    </svg>
                  ) : (
                    <svg viewBox="0 0 44 44" width="38" height="38" style={{display:"block",margin:"0 auto"}}>
                      <rect x="6" y="13" width="32" height="20" rx="4" fill="none" stroke={svgColor} strokeWidth="1.8"/>
                      <rect x="8" y="7" width="9" height="7" rx="2.5" fill="none" stroke={svgColorChair} strokeWidth="1.4"/>
                      <rect x="27" y="7" width="9" height="7" rx="2.5" fill="none" stroke={svgColorChair} strokeWidth="1.4"/>
                      <rect x="8" y="32" width="9" height="7" rx="2.5" fill="none" stroke={svgColorChair} strokeWidth="1.4"/>
                      <rect x="27" y="32" width="9" height="7" rx="2.5" fill="none" stroke={svgColorChair} strokeWidth="1.4"/>
                      {isFullyBooked && <><line x1="14" y1="18" x2="30" y2="28" stroke="rgba(239,68,68,.4)" strokeWidth="1.5"/><line x1="30" y1="18" x2="14" y2="28" stroke="rgba(239,68,68,.4)" strokeWidth="1.5"/></>}
                    </svg>
                  );
                  return (
                    <div key={t.id}
                      onClick={()=>{
                        if(!isFullyBooked){
                          if(isSelected){ setSelTbl(null); }
                          else {
                            setSelTbl(t);
                            // Tarix seçilməyibsə date picker-ə diqqət çək
                            if(!selDate){
                              setDateAlertPulse(true);
                              setTimeout(()=>setDateAlertPulse(false), 2000);
                              setTimeout(()=>datePickerRef.current?.scrollIntoView({behavior:"smooth",block:"center"}), 50);
                            } else {
                              // Tarix varsa time slot-a scroll et
                              setTimeout(()=>timeSlotRef.current?.scrollIntoView({behavior:"smooth",block:"start"}), 100);
                            }
                          }
                        }
                      }}
                      className={isSelected?"nx-card-sel":isFullyBooked?"nx-card-booked":"nx-card-free"}
                      style={{position:"relative"}}>
                      {tableShape}
                      <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:"1rem",color:isSelected?"#c8a96e":isFullyBooked?"rgba(239,68,68,.6)":"rgba(255,255,255,.85)",marginTop:"5px"}}>
                        {t.label}
                      </div>
                      <div style={{fontSize:".68rem",color:"rgba(255,255,255,.35)",marginTop:"2px"}}>👥 {t.seats} nf</div>
                      {t.zone&&<div style={{fontSize:".62rem",color:"rgba(255,255,255,.25)",marginTop:"1px"}}>{t.zone}</div>}
                      {selDate&&!isFullyBooked&&(
                        <div style={{marginTop:".35rem",background:isSelected?"rgba(200,169,110,.15)":"rgba(255,255,255,.07)",borderRadius:"20px",padding:".12rem .5rem",fontSize:".62rem",fontWeight:700,color:isSelected?"#c8a96e":"rgba(255,255,255,.4)"}}>
                          {freeSlots.length} boş
                        </div>
                      )}
                      {selDate&&isFullyBooked&&(
                        <div style={{marginTop:".35rem",background:"rgba(239,68,68,.08)",borderRadius:"20px",padding:".12rem .5rem",fontSize:".62rem",fontWeight:700,color:"#f87171"}}>Tam dolu</div>
                      )}
                      {isSelected&&(
                        <div style={{position:"absolute",top:"-6px",right:"-6px",background:"#c8a96e",color:"#0a1520",borderRadius:"50%",width:"18px",height:"18px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:".65rem",fontWeight:800}}>✓</div>
                      )}
                    </div>
                  );
                })}
              ...
              {selTable&&(()=>{
                return (
                  <div ref={timeSlotRef} style={{background:"rgba(200,169,110,.08)",border:"1.5px solid rgba(200,169,110,.3)",borderRadius:"14px",padding:"1.2rem",marginBottom:"1.2rem"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}>
                      <div>
                        <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:"1rem",color:"#c8a96e"}}>{selTable.label}</div>
                        {selTable.zone&&<div style={{fontSize:".72rem",color:"rgba(255,255,255,.4)"}}>📍 {selTable.zone} · 👥 {selTable.seats} nəfərlik</div>}
                      </div>
                      <button onClick={()=>setSelTbl(null)} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:"6px",padding:".3rem .7rem",fontSize:".72rem",color:"rgba(255,255,255,.4)",cursor:"pointer",fontFamily:"inherit"}}>✕</button>
                    </div>
                    ...
                    {!selDate?(
                      <div style={{background:"rgba(200,169,110,.06)",border:"1px solid rgba(200,169,110,.2)",borderRadius:"8px",padding:".75rem 1rem",fontSize:".82rem",color:"rgba(200,169,110,.7)",fontWeight:600}}>
                        ⬆️ Yuxarıdan tarix seçin — boş saatları görəsiniz
                      </div>
                    ):(
                      <>
                        {(()=>{
                          const fSlots = getTableFreeSlots(selTable.label, allRevs, selDate, selRest?.hours, selRest, selDuration);
                          const bSlots = ALL_TIME_SLOTS.length - fSlots.length;
                          return (
                            <div style={{display:"flex",gap:"1rem",marginBottom:".6rem",flexWrap:"wrap"}}>
                              <span style={{fontSize:".72rem",fontWeight:700,color:"rgba(255,255,255,.5)",background:"rgba(255,255,255,.06)",borderRadius:"20px",padding:".2rem .7rem"}}>✓ {fSlots.length} boş</span>
                              {bSlots>0&&<span style={{fontSize:".72rem",fontWeight:700,color:"#f87171",background:"rgba(239,68,68,.08)",borderRadius:"20px",padding:".2rem .7rem"}}>✗ {bSlots} dolu</span>}
                            </div>
                          );
                        })()}
                        {(()=>{
                          const allSlots = getTableAllSlots(selTable.label, allRevs, selDate, selRest?.hours, selRest, selDuration);
                          const hasAnyFree = allSlots.some(s=>s.status==="free");
                          if(!hasAnyFree) return (
                            <div style={{background:"rgba(239,68,68,.06)",border:"1px solid rgba(239,68,68,.18)",borderRadius:"8px",padding:".75rem",fontSize:".82rem",color:"#f87171",fontWeight:600,textAlign:"center"}}>
                          ...
                          return (
                            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(80px,1fr))",gap:".4rem"}}>
                              {allSlots.map(({slot,status})=>{
                                ...
                                return (
                                  <button key={slot}
                                    onClick={()=>{ if(!isBooked){ setSelTime(slot); setTimeout(()=>davametRef.current?.scrollIntoView({behavior:"smooth",block:"center"}), 150); } }}
                                    disabled={isBooked}
                                    className={isSel?"nx-slot-sel":isBooked?"nx-slot-booked":"nx-slot-free"}
                                    style={{position:"relative"}}>
                                    {slot}
                                    {isBooked&&<span style={{position:"absolute",top:"-5px",right:"-3px",fontSize:".5rem",background:"rgba(239,68,68,.7)",color:"#fff",borderRadius:"4px",padding:"0 .25rem",fontWeight:700}}>Dolu</span>}
                                  </button>
                                );
                              })}
                            </div>
              ...
              {!selTable&&(
                <div style={{background:"rgba(255,255,255,.03)",border:"1.5px dashed rgba(255,255,255,.1)",borderRadius:"12px",padding:"1.2rem",textAlign:"center",color:"rgba(255,255,255,.35)",marginBottom:"1.2rem"}}>
                  <div style={{fontSize:"1.5rem",marginBottom:".4rem"}}>👆</div>
                  <div style={{fontWeight:600,marginBottom:".2rem",color:"rgba(255,255,255,.6)"}}>Masaya basın</div>
              ...
              {(selRest.lat||selRest.location)&&(()=>{
                const hasCoords=selRest.lat&&selRest.lng;
                const mapsQ=hasCoords?`${selRest.lat},${selRest.lng}`:encodeURIComponent((selRest.location||selRest.name)+", Bakı, Azərbaycan");
                const navUrl=hasCoords
                  ?`https://www.google.com/maps/dir/?api=1&destination=${selRest.lat},${selRest.lng}`
                  :`https://www.google.com/maps/dir/?api=1&destination=${mapsQ}`;
                const embedUrl=hasCoords
                  ?`https://www.openstreetmap.org/export/embed.html?bbox=${parseFloat(selRest.lng)-.012},${parseFloat(selRest.lat)-.012},${parseFloat(selRest.lng)+.012},${parseFloat(selRest.lat)+.012}&layer=mapnik&marker=${selRest.lat},${selRest.lng}`
                  :`https://maps.google.com/maps?q=${mapsQ}&output=embed&z=15&hl=az`;
                const ac=selRest.accent||"#c8a96e";
                return (
                  <div style={{marginBottom:"1.2rem",borderRadius:"20px",overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,.15)",border:"1px solid rgba(200,169,110,.15)"}}>
                    <style>{`.loc-map-nav{transition:all .2s cubic-bezier(.34,1.56,.64,1)!important}.loc-map-nav:hover{transform:translateY(-3px) scale(1.02)!important;box-shadow:0 12px 32px rgba(200,169,110,.4)!important}`}</style>
                    {/* Map preview */}
                    <div style={{position:"relative",height:"140px",overflow:"hidden",background:"#0a1520"}}>
                      <iframe
                        title="loc" width="100%" height="220"
                        style={{border:0,display:"block",marginTop:"-40px",filter:"saturate(1.15) contrast(1.05) brightness(.85)"}}
                        loading="lazy" src={embedUrl}
                      />
                      {/* Gradient overlays */}
                      <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 25%,rgba(8,16,28,.92))",pointerEvents:"none"}}/>
                      <div style={{position:"absolute",inset:0,background:"linear-gradient(to right,rgba(8,16,28,.3),transparent 40%,transparent 60%,rgba(8,16,28,.3))",pointerEvents:"none"}}/>
                      {/* Restaurant chip */}
                      <div style={{position:"absolute",bottom:"14px",left:"50%",transform:"translateX(-50%)",display:"flex",flexDirection:"column",alignItems:"center",pointerEvents:"none",whiteSpace:"nowrap"}}>
                        <div style={{background:"rgba(8,16,28,.9)",backdropFilter:"blur(12px)",border:`1.5px solid ${ac}50`,borderRadius:"12px",padding:".38rem 1rem",fontSize:".75rem",fontWeight:800,color:"#fff",display:"flex",alignItems:"center",gap:".45rem",boxShadow:`0 6px 20px rgba(0,0,0,.5),0 0 0 1px ${ac}15`}}>
                          <span style={{fontSize:"1rem"}}>{selRest.emoji||"🍽️"}</span>
                          {selRest.name}
                          <svg width="10" height="10" viewBox="0 0 24 24" fill={ac}><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                        </div>
                        <svg width="10" height="6" viewBox="0 0 10 6"><polygon points="0,0 10,0 5,6" fill="rgba(8,16,28,.9)"/></svg>
                      </div>
                      {/* GPS badge */}
                      {hasCoords&&(
                        <div style={{position:"absolute",top:"10px",left:"10px",display:"flex",alignItems:"center",gap:".3rem",background:"rgba(8,16,28,.75)",backdropFilter:"blur(10px)",border:"1px solid rgba(255,255,255,.1)",borderRadius:"100px",padding:".25rem .65rem",fontSize:".6rem",fontWeight:600,color:"rgba(255,255,255,.6)"}}>
                          <svg width="7" height="7" viewBox="0 0 24 24" fill={ac}><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/></svg>
                          GPS · Dəqiq
                        </div>
                      )}
                    </div>
                    {/* Bottom info + button */}
                    <div style={{background:"linear-gradient(180deg,#08101c,#0d1828)",padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"10px"}}>
                      <div style={{flex:1,minWidth:0}}>
                        {selRest.location&&(
                          <div style={{display:"flex",alignItems:"center",gap:"5px",marginBottom:"3px"}}>
                            <svg width="10" height="10" viewBox="0 0 24 24" fill={ac}><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                            <span style={{fontSize:".78rem",color:"rgba(255,255,255,.8)",fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{selRest.location}</span>
                          </div>
                        )}
                        {selRest.hours&&(
                          <div style={{display:"flex",alignItems:"center",gap:"5px"}}>
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="rgba(255,255,255,.4)"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm.5 5v5.25l4.5 2.67-.75 1.23L11 13V7h1.5z"/></svg>
                            <span style={{fontSize:".72rem",color:"rgba(255,255,255,.45)",fontWeight:400}}>{selRest.hours}</span>
                          </div>
                        )}
                      </div>
                      <a href={navUrl} target="_blank" rel="noreferrer" className="loc-map-nav"
                        style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"5px",background:`linear-gradient(135deg,${ac},#e0c07a)`,color:"#0a1520",borderRadius:"14px",padding:"10px 16px",textDecoration:"none",flexShrink:0,boxShadow:`0 6px 20px ${ac}40`}}>
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="#0a1520"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                        <span style={{fontSize:".72rem",fontWeight:900,letterSpacing:"-.01em",whiteSpace:"nowrap"}}>Yol Tap</span>
                      </a>
                    </div>
                  </div>
                );
              ...
              <button className="btnp" onClick={()=>navigateStep("menu")} disabled={!selTable||!selTime||!selDate}
                ref={davametRef}
                style={{background:selTable&&selTime&&selDate?"linear-gradient(135deg,#c8a96e,#e0c07a)":"rgba(255,255,255,.06)",color:selTable&&selTime&&selDate?"#0a1520":"rgba(255,255,255,.25)",border:"none",padding:".9rem 2rem",fontFamily:"inherit",fontSize:".95rem",fontWeight:700,cursor:selTable&&selTime&&selDate?"pointer":"not-allowed",borderRadius:"10px",width:"100%"}}>
                Davam et — Menyu →
              </button>
            </>
          )}
      ...
      {step==="menu"&&selRest&&selTable&&(
        <div style={{maxWidth:"720px",margin:"0 auto",padding:"1rem"}}>
          ...
          <div style={{fontWeight:800,fontSize:"1.5rem",letterSpacing:"-.02em",marginBottom:".3rem",color:"#fff"}}>Əvvəlcədən sifariş</div>
          <div style={{fontSize:".75rem",color:"rgba(255,255,255,.35)",marginBottom:"1.8rem"}}>Gəlincə hazır olsun (istəyə görə)</div>
          {menuItems.length===0?(
            <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:"14px",textAlign:"center",padding:"2rem",marginBottom:"1.5rem"}}>
              <div style={{fontSize:"1.5rem",marginBottom:".5rem"}}>🍽️</div>
              <div style={{color:"rgba(255,255,255,.35)"}}>Bu restoran üçün menyu əlavə edilməyib</div>
            </div>
          ):(
            <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:"14px",overflow:"hidden",marginBottom:"1.5rem"}}>
              {categories.map((cat,ci)=>(
                <div key={cat}>
                  {ci>0&&<div style={{height:"1px",background:"rgba(255,255,255,.06)"}}/>}
                  <div style={{padding:".7rem 1.2rem",background:"rgba(255,255,255,.03)",fontSize:".62rem",fontWeight:700,letterSpacing:".12em",textTransform:"uppercase",color:"rgba(200,169,110,.5)"}}>{cat}</div>
                  {menuItems.filter(i=>i.cat===cat).map((item,idx,arr)=>(
                    <div key={item.id||idx} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:".9rem 1.2rem",borderBottom:idx<arr.length-1?"1px solid rgba(255,255,255,.05)":"none",background:cart[item.id]?"rgba(200,169,110,.08)":"transparent"}}>
                      <div style={{display:"flex",alignItems:"center",gap:".8rem"}}>
                        <span style={{fontSize:"1.3rem"}}>{item.emoji||"🍽️"}</span>
                        <div>
                          <div style={{fontWeight:600,fontSize:".92rem",color:"rgba(255,255,255,.85)"}}>{item.name}</div>
                          <div style={{fontSize:".72rem",color:"rgba(255,255,255,.35)"}}>{item.desc}</div>
                        </div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:".8rem"}}>
                        <span style={{fontWeight:700,fontSize:".9rem",color:"#c8a96e"}}>
                          {selRest?.discount_percent > 0 ? (
                            <>
                              <span style={{textDecoration:"line-through",opacity:.5,fontSize:".78rem",color:"rgba(255,255,255,.4)",marginRight:"4px"}}>{item.price}₼</span>
                              <span style={{color:"#f87171"}}>{(item.price*(1-selRest.discount_percent/100)).toFixed(2)}₼</span>
                            </>
                          ) : `${item.price}₼`}
                        </span>
                        <div style={{display:"flex",alignItems:"center",gap:".4rem"}}>
                          <button onClick={()=>removeItem(item)} style={{width:"28px",height:"28px",background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.12)",color:"rgba(255,255,255,.6)",cursor:"pointer",borderRadius:"7px",fontSize:"1.1rem",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                          <span style={{minWidth:"20px",textAlign:"center",fontWeight:700,fontSize:".9rem",color:cart[item.id]?"#c8a96e":"rgba(255,255,255,.2)"}}>{cart[item.id]||0}</span>
                          <button onClick={()=>addItem(item)} style={{width:"28px",height:"28px",background:"rgba(200,169,110,.15)",border:"1px solid rgba(200,169,110,.3)",color:"#c8a96e",cursor:"pointer",borderRadius:"7px",fontSize:"1.1rem",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:"12px",padding:"1.2rem",marginBottom:"1.5rem"}}>
            <div style={{fontSize:".65rem",fontWeight:700,letterSpacing:".12em",textTransform:"uppercase",color:"rgba(200,169,110,.5)",marginBottom:".9rem"}}>Rezervasiya xülasəsi</div>
            <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:".4rem 1rem",fontSize:".875rem"}}>
              {[["Ad",custName],["Telefon",custPhone],["Restoran",selRest.name],["Tarix",selDate],["Saat",selTime],["Masa",`${selTable.label} (${selTable.seats} nf)`],["Qonaqlar",`${guests} nəfər`]].map(([k,v])=>(
                <Fragment key={k}><span style={{color:"rgba(255,255,255,.35)"}}>{k}:</span><span style={{fontWeight:600,color:"rgba(255,255,255,.8)"}}>{v}</span></Fragment>
              ))}
              {cartCount>0&&<Fragment><span style={{color:"rgba(255,255,255,.35)"}}>Ön sifariş:</span><span style={{fontWeight:700,color:"#c8a96e"}}>{cartTotal}₼</span></Fragment>}
            </div>
          </div>
          {saveError&&<div style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",borderRadius:"8px",padding:".75rem 1rem",marginBottom:"1rem",color:"#f87171",fontSize:".85rem"}}>{saveError}</div>}
          <div style={{display:"flex",gap:".8rem",flexWrap:"wrap"}}>
            <button className="btnp" onClick={()=>navigateStep("datetime")} style={{background:"linear-gradient(135deg,#c8a96e,#e0c07a)",color:"#0a1520",border:"none",padding:".9rem 1.8rem",fontFamily:"inherit",fontSize:".92rem",fontWeight:700,cursor:"pointer",borderRadius:"10px"}}>
              Davam et — Məlumatlar →
            </button>
            <button onClick={()=>navigateStep("datetime")} style={{background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",color:"rgba(255,255,255,.5)",padding:".9rem 1.3rem",fontFamily:"inherit",fontSize:".85rem",cursor:"pointer",borderRadius:"10px",fontWeight:500}}>Sifariş olmadan keç</button>
          </div>
        </div>
      )}
      ...
      {/* ── OTP Modal ── */}
      {otpStep&&(
        <div style={{position:"fixed",inset:0,background:"rgba(5,13,23,.92)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
          <div style={{background:"linear-gradient(160deg,#0a1f35,#0f2d4a)",border:"1px solid rgba(200,169,110,.25)",borderRadius:"20px",padding:"2rem",maxWidth:"380px",width:"100%",textAlign:"center"}}>
            <div style={{fontSize:"2.5rem",marginBottom:"1rem"}}>🔐</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:"1.3rem",color:"#fff",marginBottom:".4rem"}}>Telefonu təsdiq edin</div>
            <div style={{fontSize:".82rem",color:"rgba(255,255,255,.45)",marginBottom:"1.5rem"}}>
              <span style={{color:"#c8a96e",fontWeight:600}}>{custPhone}</span> nömrəsinə 4 rəqəmli kod göndərildi
            </div>
            <div style={{display:"flex",gap:".5rem",justifyContent:"center",marginBottom:"1rem"}}>
              {[0,1,2,3].map(i=>(
                <div key={i} style={{width:"52px",height:"60px",background:"rgba(255,255,255,.07)",border:`2px solid ${otpInput.length===i?"rgba(200,169,110,.8)":"rgba(255,255,255,.12)"}`,borderRadius:"12px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.5rem",fontWeight:700,color:"#c8a96e",transition:"border .15s"}}>
                  {otpInput[i]||""}
                </div>
              ))}
            </div>
            <input
              type="number" maxLength={4} value={otpInput}
              onChange={e=>{ const v=e.target.value.replace(/\D/g,"").slice(0,4); setOtpInput(v); setOtpError(""); }}
              onKeyDown={e=>e.key==="Enter"&&otpInput.length===4&&verifyOTP()}
              placeholder="0000"
              autoFocus
              style={{width:"100%",background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.15)",borderRadius:"10px",padding:".8rem",fontSize:"1.2rem",color:"#fff",textAlign:"center",outline:"none",fontFamily:"inherit",letterSpacing:".3em",marginBottom:".8rem"}}
            />
            {otpError&&<div style={{color:"#f87171",fontSize:".82rem",marginBottom:".8rem",fontWeight:600}}>{otpError}</div>}
            <button onClick={verifyOTP} disabled={otpInput.length!==4}
              style={{width:"100%",background:otpInput.length===4?"linear-gradient(135deg,#c8a96e,#e0c07a)":"rgba(255,255,255,.06)",color:otpInput.length===4?"#0a1520":"rgba(255,255,255,.25)",border:"none",borderRadius:"10px",padding:".85rem",fontFamily:"inherit",fontSize:".95rem",fontWeight:700,cursor:otpInput.length===4?"pointer":"not-allowed",marginBottom:".75rem",transition:"all .2s"}}>
              ✓ Təsdiq Et
            </button>
            <div style={{fontSize:".75rem",color:"rgba(255,255,255,.3)"}}>
              ...
              <span onClick={async()=>{
                setOtpSending(true);
                const code=generateOTP(); setOtpCode(code); setOtpInput(""); setOtpError("");
                await sendWA(custPhone,`🔐 *1rezerv Təsdiq Kodu*\n\nKodunuz: *${code}*`);
                setOtpSending(false);
              }} style={{color:"#c8a96e",cursor:"pointer",fontWeight:600}}>
                {otpSending?"Göndərilir...":"Yenidən göndər"}
              </span>
            </div>
          </div>
        </div>
      ...
      {step==="success"&&(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:"3rem 2rem",textAlign:"center"}}>
          <div style={{width:"72px",height:"72px",borderRadius:"50%",background:"rgba(34,197,94,.12)",border:"2px solid rgba(34,197,94,.4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2rem",marginBottom:"1.2rem",color:"#4ade80"}}>✓</div>
          <div style={{fontSize:"1.8rem",fontWeight:800,letterSpacing:"-.02em",marginBottom:".4rem",color:"#fff"}}>Rezervasiya Təsdiqləndi!</div>
          <div style={{color:"rgba(255,255,255,.4)",marginBottom:"1rem"}}>Kod: <span style={{fontWeight:700,color:"#c8a96e",background:"rgba(200,169,110,.12)",border:"1px solid rgba(200,169,110,.25)",padding:".2rem .8rem",borderRadius:"8px",fontSize:"1.1rem"}}>#{bookingCode}</span></div>
          <div style={{background:"rgba(255,255,255,.06)",border:"1.5px solid rgba(255,255,255,.12)",borderRadius:"16px",padding:"1.2rem",marginBottom:"1rem",display:"inline-block"}}>
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent("1REZERV:"+bookingCode)}&bgcolor=0a1520&color=c8a96e&margin=8`} alt="QR" style={{width:"180px",height:"180px",borderRadius:"8px",display:"block"}}/>
            <div style={{marginTop:".6rem",fontSize:".72rem",color:"rgba(255,255,255,.35)"}}>Restoranda bu QR-i göstərin</div>
          </div>
          <div style={{background:"rgba(200,169,110,.08)",border:"1px solid rgba(200,169,110,.2)",borderRadius:"10px",padding:".65rem 1.2rem",marginBottom:"1.2rem",fontSize:".8rem",color:"rgba(200,169,110,.8)",fontWeight:500}}>📱 Ekran görüntüsü çəkin — restoranda oxudacaqlar</div>
          <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:"14px",padding:"1.4rem 1.8rem",marginBottom:"1.5rem",textAlign:"left",minWidth:"300px"}}>
            <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:".5rem 1rem",fontSize:".875rem"}}>
              {[["Ad",custName],["Telefon",custPhone],["Restoran",selRest?.name],["Tarix",selDate],["Saat",selTime],["Masa",selTable?.label],["Qonaqlar",`${guests} nəfər`]].map(([k,v])=>(
                <Fragment key={k}><span style={{color:"rgba(255,255,255,.35)"}}>{k}:</span><span style={{fontWeight:600,color:"rgba(255,255,255,.85)"}}>{v}</span></Fragment>
              ))}
              {cartTotal>0&&<Fragment><span style={{color:"rgba(255,255,255,.35)"}}>Ön sifariş:</span><span style={{fontWeight:700,color:"#c8a96e"}}>{cartTotal}₼</span></Fragment>}
              {specialOccasion&&<Fragment><span style={{color:"rgba(255,255,255,.35)"}}>Xüsusi gün:</span><span style={{fontWeight:700,color:"#c8a96e"}}>{specialOccasion}</span></Fragment>}
            </div>
          </div>
          {specialOccasion&&<div style={{background:"rgba(200,169,110,.08)",border:"1px solid rgba(200,169,110,.2)",borderRadius:"10px",padding:".75rem 1.2rem",marginBottom:"1rem",fontSize:".85rem",color:"#c8a96e",fontWeight:600}}>🎊 Xüsusi gününüz münasibətilə restoran xüsusi hazırlıq edəcək!</div>}
          <button className="btnp" onClick={reset} style={{background:"linear-gradient(135deg,#c8a96e,#e0c07a)",color:"#0a1520",border:"none",padding:".9rem 2rem",fontFamily:"inherit",fontSize:".92rem",fontWeight:700,cursor:"pointer",borderRadius:"10px"}}>Yeni Rezervasiya</button>
        </div>
      )}
    </div>
  );
