      ...
      <div style={{ maxWidth:"800px", margin:"0 auto", padding:"2rem" }}>
        ...
        {(rest.phone || rest.hours || rest.location) && (
          <div style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:"16px", overflow:"hidden", marginBottom:"1.5rem" }}>
            <div style={{ padding:"1.2rem 1.4rem", borderBottom:"1px solid #F1F5F9", fontWeight:700, fontSize:".95rem", color:"#1a1a2e" }}>📋 Məlumat</div>
            <div style={{ padding:"1rem 1.4rem", display:"flex", flexDirection:"column", gap:".7rem" }}>
              {rest.phone && (
                <div style={{ display:"flex", alignItems:"center", gap:".75rem" }}>
                  <div style={{ width:"32px", height:"32px", borderRadius:"8px", background:"#e8f2fb", display:"flex", alignItems:"center", justifyContent:"center", fontSize:".9rem", flexShrink:0 }}>📞</div>
                  <div>
                    <div style={{ fontSize:".68rem", color:"#94A3B8", fontWeight:600, textTransform:"uppercase", letterSpacing:".08em", marginBottom:".1rem" }}>Əlaqə nömrəsi</div>
                    <a href={"tel:"+rest.phone} style={{ fontWeight:700, fontSize:".9rem", color:"#12487a", textDecoration:"none" }}>{rest.phone}</a>
                  </div>
                </div>
              )}
              {rest.hours && (()=>{
                ...
                const match = rest.hours.replace(/\s/g,"").match(/(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})/);
                let isOpen = null;
                if(match){ const o=parseInt(match[1])*60+parseInt(match[2]); const c=parseInt(match[3])*60+parseInt(match[4]); isOpen = hm>=o && hm<c; }
                return (
                  <div style={{ display:"flex", alignItems:"center", gap:".75rem" }}>
                    <div style={{ width:"32px", height:"32px", borderRadius:"8px", background: isOpen===null?"#F8FAFC":isOpen?"#F0FDF4":"#FEF2F2", display:"flex", alignItems:"center", justifyContent:"center", fontSize:".9rem", flexShrink:0 }}>🕐</div>
                    <div>
                      <div style={{ fontSize:".68rem", color:"#94A3B8", fontWeight:600, textTransform:"uppercase", letterSpacing:".08em", marginBottom:".1rem" }}>İş saatları</div>
                      <div style={{ display:"flex", alignItems:"center", gap:".5rem" }}>
                        <span style={{ fontWeight:700, fontSize:".9rem", color:"#1a1a2e" }}>{rest.hours}</span>
                        {isOpen !== null && <span style={{ fontSize:".72rem", fontWeight:700, padding:".15rem .55rem", borderRadius:"20px", background:isOpen?"#F0FDF4":"#FEF2F2", color:isOpen?"#16A34A":"#DC2626" }}>{isOpen?"● Açıqdır":"● Bağlıdır"}</span>}
                      </div>
                    </div>
                  </div>
                );
              ...
              {rest.location && (
                <div style={{ display:"flex", alignItems:"center", gap:".75rem" }}>
                  <div style={{ width:"32px", height:"32px", borderRadius:"8px", background:"#FFF7ED", display:"flex", alignItems:"center", justifyContent:"center", fontSize:".9rem", flexShrink:0 }}>📍</div>
                  <div>
                    <div style={{ fontSize:".68rem", color:"#94A3B8", fontWeight:600, textTransform:"uppercase", letterSpacing:".08em", marginBottom:".1rem" }}>Ünvan</div>
                    <a href={"https://maps.google.com?q="+encodeURIComponent(rest.location+", Bakı, Azərbaycan")} target="_blank" rel="noreferrer" style={{ fontWeight:600, fontSize:".9rem", color:"#12487a", textDecoration:"none" }}>{rest.location} ↗</a>
                  </div>
                </div>
              )}
            </div>
            ...
            {rest.location && (
              <div style={{ borderTop:"1px solid #F1F5F9" }}>
                <iframe
                  ...
                  loading="lazy"
                  allowFullScreen
                  src={"https://maps.google.com/maps?q="+encodeURIComponent(rest.location+", Bakı, Azərbaycan")+"&output=embed&z=15"}
                />
              </div>
            )}
          </div>
