...
function HomePage({ navigate, search: searchProp }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setRests((r1.data || []).map(r => ({
        ...r, priceRange: r.price_range || "₼₼",
        tags: Array.isArray(r.tags) ? r.tags : []
      })));
      setAllReviews(r2.data || []);
      const total = (r3.data || []).length;
      const booked = (r4.data || []).length;
  ...
  // Bütün unikal kateqoriyaları topla (dublikatları birləşdir)
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:"#f0ece4", minHeight:"100vh", color:"#1a1a2e", overflowX:"hidden" }}>
      ...
      {/* ── HEADER ── */}
      <style>{`
        .hdr-btn { display:flex; align-items:center; gap:.3rem; border-radius:8px; font-family:inherit; cursor:pointer; white-space:nowrap; transition:opacity .15s; }
        .hdr-btn:hover { opacity:.8; }
        .hdr-label { display:inline; }
        @media(max-width:480px){
          .hdr-label { display:none; }
          .hdr-btn { padding:.38rem .55rem !important; font-size:.8rem !important; }
        }
      `}</style>
      <header style={{ background:"#fff", position:"sticky", top:0, zIndex:200, height:"56px", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 1rem", boxShadow:"0 1px 0 rgba(0,0,0,.08)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:".6rem", cursor:"pointer", flexShrink:0 }} onClick={() => navigate("/")}>
          <div style={{ width:"34px", height:"34px", borderRadius:"9px", background:"#1a1a2e", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1rem", flexShrink:0 }}>🍽️</div>
          <div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:"1rem", letterSpacing:"-.01em", lineHeight:1.1 }}>MasaAz</div>
            <div style={{ fontSize:".48rem", letterSpacing:".12em", textTransform:"uppercase", color:"#9c9c9c", marginTop:"1px" }}>Rezervasiya</div>
          </div>
        </div>
        <div style={{ display:"flex", gap:".35rem", alignItems:"center" }}>
          <button onClick={() => navigate("/cancel")} className="hdr-btn"
            style={{ background:"none", border:"1.5px solid #e0ddd8", color:"#6b6b6b", padding:".38rem .75rem", fontSize:".76rem", fontWeight:500 }}>
            ✕ <span className="hdr-label">Ləğv et</span>
          </button>
          <button onClick={() => navigate("/onboard")} className="hdr-btn"
            style={{ background:"#1a1a2e", color:"#fff", border:"none", padding:".4rem .75rem", fontSize:".76rem", fontWeight:600 }}>
            + <span className="hdr-label">Qeydiyyat</span>
          </button>
          <a href="/admin.html" className="hdr-btn"
            style={{ background:"none", border:"1.5px solid #e0ddd8", color:"#6b6b6b", padding:".38rem .75rem", fontSize:".76rem", fontWeight:500, textDecoration:"none" }}>
      ...
      <div style={{ background:"linear-gradient(160deg,#0d1b2a 0%,#1a2d42 45%,#12345a 100%)", minHeight:"480px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"4rem 2rem 3rem", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(2.6rem,5.5vw,4.2rem)", fontWeight:700, lineHeight:1.15, color:"#c8a96e", marginBottom:"1.5rem", marginTop:"-.4rem" }}>
          rezerv edin
        </h1>
          ...
          {filtered.map(r => (
            ...
            >
              ...
              <div style={{ position:"relative", height:"200px", background: r.cover_url ? "transparent" : `linear-gradient(135deg, ${r.accent||"#1a2d42"}cc, ${r.accent||"#0d1b2a"}ee)`, overflow:"hidden" }}>
                ...
                {/* Rating badge */}
                {(()=>{ const rt = getRating(r.name); return rt ? (
                <div style={{ position:"absolute", top:"12px", right:"12px", background:"rgba(255,255,255,.92)", borderRadius:"50px", padding:".28rem .7rem", display:"flex", alignItems:"center", gap:".3rem", fontSize:".78rem", fontWeight:700, color:"#1a1a2e" }}>
                  <span style={{ color:"#c8a96e" }}>★</span> {rt}
                </div>) : null; })()}
                {/* Status badge */}
                {!r.available && (
                ...
                <div style={{ position:"absolute", bottom:"12px", left:"12px" }}>
                  <span style={{ fontSize:".65rem", letterSpacing:".1em", textTransform:"uppercase", color:"rgba(255,255,255,.7)", fontWeight:500 }}>{normCuisine(r.cuisine)}</span>
                </div>
              </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:".9rem",...box,marginBottom:"1rem",flexWrap:"wrap"}}>
            ...
            <div style={{position:"relative"}}>
              <button onClick={()=>setCalOpen(p=>!p)}
                style={{display:"flex",alignItems:"center",gap:".6rem",background:selDate?"#e8f2fb":"#F8FAFC",border:`1.5px solid ${selDate?"#12487a":"#E2E8F0"}`,borderRadius:"10px",padding:".6rem 1rem",cursor:"pointer",fontFamily:"inherit",fontSize:".85rem",fontWeight:600,color:selDate?"#12487a":"#64748B",whiteSpace:"nowrap"}}>
                📅 {selDate ? new Date(selDate+"T12:00:00").toLocaleDateString("az-AZ",{day:"numeric",month:"long",year:"numeric"}) : "Tarix seçin"}
                <span style={{fontSize:".7rem",color:"#94A3B8"}}>{calendarOpen?"▲":"▼"}</span>
              </button>
              {calendarOpen&&(
