...
function MainApp() {
  ...
  useEffect(()=>{
    supabase.from("restaurants").select("*").order("name").then(({data})=>{
      setRests((data||[]).map(r=>({...r,priceRange:r.price_range||"₼₼",tags:Array.isArray(r.tags)?r.tags:[]})));
      setLR(false);
    });
  },[]);
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#F8FAFC",minHeight:"100vh",color:"#0F172A"}}>
                  ...
                  {dbTables.map(t=>{
                    ...
                    return (
                      <div key={t.id} className="tsvg" style={{position:"absolute",left:`${t.x||10}%`,top:`${t.y||10}%`,transform:"translate(-50%,-50%)",cursor:isBooked?"not-allowed":"pointer",opacity:isBooked?0.55:1,filter:isSelected?"drop-shadow(0 2px 8px rgba(37,99,235,.25))":isHovered?"drop-shadow(0 2px 6px rgba(0,0,0,.1))":"none"}}
                        onClick={()=>!isBooked&&setSelTbl(isSelected?null:t)} onMouseEnter={()=>setHover(t)} onMouseLeave={()=>setHover(null)}>
                ...
                <div style={{display:"grid",gridTemplateColumns:preview.img?"180px 1fr":"1fr",overflow:"hidden",background:"#fff",border:`1.5px solid ${selTable?.id===preview.id?"#3B82F6":"#E2E8F0"}`,borderRadius:"12px",marginBottom:"1.2rem"}}>
                  ...
                  <div style={{padding:"1.1rem 1.2rem",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
                    ...
                    {!bookedLabels.includes(preview.label)&&(
                      <button className="btnp" onClick={()=>setSelTbl(selTable?.id===preview.id?null:preview)} style={{background:selTable?.id===preview.id?"#16A34A":"#0F172A",color:"#fff",border:"none",padding:".55rem 1rem",fontFamily:"inherit",fontSize:".76rem",fontWeight:700,cursor:"pointer",borderRadius:"8px",width:"fit-content",marginTop:".7rem"}}>
      ...
      {step==="success"&&(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:"3rem 2rem",textAlign:"center"}}>
          ...
          <div style={{color:"#64748B",marginBottom:"1.2rem"}}>Kod: <span style={{fontWeight:700,color:"#0F172A",background:"#F1F5F9",padding:".2rem .6rem",borderRadius:"6px",fontSize:"1.1rem"}}>#{bookingCode}</span></div>
          <div style={{background:"#FEF9C3",border:"1px solid #FDE047",borderRadius:"10px",padding:".7rem 1.2rem",marginBottom:"1.5rem",fontSize:".82rem",color:"#92400E",fontWeight:500}}>💡 Bu kodu yadda saxlayın — ləğv etmək lazım olarsa lazım olacaq</div>
