    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#050d17,#0a1f35,#0f2d4a)",fontFamily:"'DM Sans',system-ui,sans-serif",color:"#fff",paddingBottom:"120px"}}>
      ...
      <div style={{padding:"1rem",maxWidth:"600px",margin:"0 auto"}}>
        {menuItems.length===0?(
          ...
          <div key={item.id} style={{background:cart[item.id]?"rgba(200,169,110,.08)":"rgba(255,255,255,.04)",border:`1.5px solid ${cart[item.id]?"rgba(200,169,110,.3)":"rgba(255,255,255,.07)"}`,borderRadius:"14px",padding:"1rem",marginBottom:".7rem",display:"flex",alignItems:"center",gap:"1rem",transition:"all .2s"}}>
            {item.image_url?(
              <img src={item.image_url} alt={item.name} style={{width:"72px",height:"72px",borderRadius:"10px",objectFit:"cover",flexShrink:0}}/>
            ):(
              <div style={{width:"72px",height:"72px",borderRadius:"10px",background:"rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2.2rem",flexShrink:0}}>{item.emoji||"🍽️"}</div>
            )}
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontWeight:700,fontSize:".95rem",color:"rgba(255,255,255,.9)",marginBottom:".2rem"}}>{item.name}</div>
              {item.desc&&<div style={{fontSize:".75rem",color:"rgba(255,255,255,.35)",marginBottom:".4rem",lineHeight:1.4}}>{item.desc}</div>}
              <div style={{fontWeight:800,color:"#c8a96e",fontSize:".92rem"}}>{item.price}₼</div>
            </div>
