...
-- ── Platform Settings (Stripe Payment Link vs.) ──────────────
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "read settings" ON platform_settings;
  DROP POLICY IF EXISTS "write settings" ON platform_settings;
EXCEPTION WHEN OTHERS THEN NULL; END $$;
CREATE POLICY "read settings"  ON platform_settings FOR SELECT USING (true);
CREATE POLICY "write settings" ON platform_settings FOR ALL USING (true) WITH CHECK (true);

-- Default boş link
INSERT INTO platform_settings (key, value) VALUES ('stripe_payment_link','')
