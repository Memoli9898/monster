...
function getTimeSlots(hours) {
  // hours format: "15:00-02:00" or "15:00-23:00"
  ...
  if(hours) {
    ...
    if(m) {
      ...
      if(eH < startH) eH += 24;
      endH = eH - 1; // last slot 1hr before close
      if(endH > 23) endH = 23;
    }
  }
  const slots=[];
  for(let h=startH;h<=endH;h++){
    ...
    if(h<endH) slots.push(String(hh).padStart(2,"0")+":30");
  }
  return slots;
}
...
const ALL_TIME_SLOTS = getTimeSlots(); // fallback (12-22)
...
function getTableFreeSlots(tableLabel, allRevs, date, restHours) {
  const DURATION = 90;
  const slots = getTimeSlots(restHours);
...
function MainApp({ navigate, restSlug, initialStep, preTable }) {
  ...
  const submitReservationFinal = async () => {
    ...
    const DEFAULT_BOOKING_TPL = `🍽️ *{restaurant}* — Rezervasiya Alındı!\n\nSalam, {name}!\n📅 Tarix: {date}\n⏰ Saat: {time}\n👥 Nəfər: {guests}\n🪑 Masa: {table}\n🔖 Kod: *{code}*\n\nTəsdiq üçün gözləyin. ✅`;
    const tpl = localStorage.getItem("wa_tpl_booking") || DEFAULT_BOOKING_TPL;
    let waMsg = tpl
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"linear-gradient(155deg,#050d17 0%,#0a1f35 45%,#0f2d4a 100%)",minHeight:"100vh",color:"#fff"}}>
      ...
      {step==="success"&&(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:"3rem 2rem",textAlign:"center"}}>
          ...
          <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:"10px",padding:".75rem 1.2rem",marginBottom:"1rem",fontSize:".78rem",color:"rgba(255,255,255,.4)",textAlign:"center"}}>
            ⭐ Restorandakı QR kodu oxudun — rəy bildirin
          </div>
          <button className="btnp" onClick={reset} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.12)",color:"rgba(255,255,255,.7)",padding:".9rem 2rem",fontFamily:"inherit",fontSize:".92rem",fontWeight:600,cursor:"pointer",borderRadius:"10px"}}>Yeni Rezervasiya</button>
        </div>
      )}
    </div>
  );
