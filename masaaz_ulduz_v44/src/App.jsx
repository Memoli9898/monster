...
function MainApp({ navigate, restSlug, initialStep, preTable }) {
  ...
  const submitReservation = async () => {
    ...
    // 3-cü+ cəhd — blok
    if(attempts >= 3) {
      setSaveErr("⛔ Bu sessiyada bu nömrə ilə artıq 3 rezervasiya edilib.");
      return;
    }
    // 2-ci cəhd — OTP lazımdır
    if(attempts === 1) {
      setSaving(true); setSaveErr("");
    ...
    // 1-ci rezervasiya — birbaşa davam et
  ...
  const verifyOTP = () => {
    if(otpInput.trim() === otpCode) {
      ...
      incrementAttempt(custPhone); // 2-ci rezervasiya sayılır
