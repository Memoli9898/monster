...
function getResDuration() {
  return parseInt(localStorage.getItem("res_duration") || "90");
}

function getTableFreeSlots(tableLabel, allRevs, date, restHours) {
  const DURATION = getResDuration();
...
function getTableAllSlots(tableLabel, allRevs, date, restHours) {
  const DURATION = getResDuration();
  const slots = restHours ? getTimeSlots(restHours) : ALL_TIME_SLOTS;
  const occupied = allRevs
    .filter(r => r.table_label===tableLabel && r.date===date && r.status!=="ləğv edildi")
          ...
          {dbTables.length===0?(
            ...
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:".75rem",marginBottom:"1.2rem"}}>
                {dbTables.map(t=>{
                  const freeSlots = selDate ? getTableFreeSlots(t.label, allRevs, selDate, selRest?.hours) : getTimeSlots(selRest?.hours);
                ...
                return (
                  <div ref={timeSlotRef} style={{background:"rgba(200,169,110,.08)",border:"1.5px solid rgba(200,169,110,.3)",borderRadius:"14px",padding:"1.2rem",marginBottom:"1.2rem"}}>
                    ...
                    {!selDate?(
                      ...
                      <>
                        {(()=>{
                          const fSlots = getTableFreeSlots(selTable.label, allRevs, selDate, selRest?.hours);
                        ...
                        {(()=>{
                          const allSlots = getTableAllSlots(selTable.label, allRevs, selDate, selRest?.hours);
