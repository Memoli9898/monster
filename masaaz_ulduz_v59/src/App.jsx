...
// DURATION: restoran DB-dən oxunur, fallback localStorage, sonra 90 dəq
function getResDuration(rest) {
  if (rest?.res_duration) return parseInt(rest.res_duration);
  return parseInt(localStorage.getItem("res_duration") || "90");
}

function getTableFreeSlots(tableLabel, allRevs, date, restHours, rest) {
  const DURATION = getResDuration(rest);
...
function getTableAllSlots(tableLabel, allRevs, date, restHours, rest) {
  const DURATION = getResDuration(rest);
  const slots = restHours ? getTimeSlots(restHours) : ALL_TIME_SLOTS;
  const occupied = allRevs
    .filter(r => r.table_label===tableLabel && r.date===date && r.status!=="ləğv edildi")
          ...
          {dbTables.length===0?(
            ...
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:".75rem",marginBottom:"1.2rem"}}>
                {dbTables.map(t=>{
                  const freeSlots = selDate ? getTableFreeSlots(t.label, allRevs, selDate, selRest?.hours, selRest) : getTimeSlots(selRest?.hours);
                ...
                return (
                  <div ref={timeSlotRef} style={{background:"rgba(200,169,110,.08)",border:"1.5px solid rgba(200,169,110,.3)",borderRadius:"14px",padding:"1.2rem",marginBottom:"1.2rem"}}>
                    ...
                    {!selDate?(
                      ...
                      <>
                        {(()=>{
                          const fSlots = getTableFreeSlots(selTable.label, allRevs, selDate, selRest?.hours, selRest);
                        ...
                        {(()=>{
                          const allSlots = getTableAllSlots(selTable.label, allRevs, selDate, selRest?.hours, selRest);
