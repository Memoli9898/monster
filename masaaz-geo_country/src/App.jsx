...
const T={
  AZ:{
    ...
    cancelOk:"Rezervasiya ləğv edildi ✓", notFound:"Bu nömrə ilə aktiv rezervasiya tapılmadı.",
  ...
  useEffect(()=>{
    supabase.from("restaurants").select("*").order("name").then(({data})=>{
      ...
      const applyGeo=(geo,err)=>{
        let filtered=mapped;
        let cc="AZ"; // default ölkə

        if(!err&&geo){
          cc=geoCountry(geo.lat,geo.lng);
          setT(T[cc]||T.AZ);

          // 1. Koordinatı olan restoranlar — 100km filtrə
          const withCoords=mapped.filter(r=>r.lat&&r.lng);
          const near=withCoords.filter(r=>distKm(geo.lat,geo.lng,r.lat,r.lng)<=100);

          // 2. Koordinatsız restoranlar — ölkə koduna görə filtrə
          const byCountry=mapped.filter(r=>(!r.lat||!r.lng)&&r.country===cc);

          // 3. Birləşdir — dublikat olmadan
          const combined=[...near,...byCountry.filter(b=>!near.find(n=>n.id===b.id))];

          // Əgər heç nə tapılmadısa — yalnız eyni ölkəni göstər
          filtered = combined.length>0
            ? combined
            : mapped.filter(r=>r.country===cc);

          // Əgər yenə boşdursa — hamısını göstər
          ...
          // GPS yoxdursa — browser dilinə görə ölkə
          const lang=navigator.language||"az";
          if(lang.startsWith("ru")) { cc="RU"; setT(T.RU); }
          else { cc="AZ"; setT(T.AZ); }
          filtered=mapped.filter(r=>r.country===cc);
          if(filtered.length===0) filtered=mapped;
        }
