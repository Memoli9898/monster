...
function HomePage({ navigate, search: searchProp }) {
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:"#eef3f8", minHeight:"100vh", color:"#12487a", overflowX:"hidden", maxWidth:"100vw" }}>
      <style>{css}</style>
      <header style={{ background:"#fff", borderBottom:"1px solid #E2E8F0", position:"sticky", top:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 1rem", height:"56px", boxShadow:"0 1px 3px rgba(0,0,0,.04)", gap:".5rem" }}>
        <div style={{ display:"flex", alignItems:"center", gap:".5rem", flexShrink:0 }}>
          <div style={{ width:"30px", height:"30px", borderRadius:"8px", background:"#12487a", display:"flex", alignItems:"center", justifyContent:"center", fontSize:".9rem" }}>🍽️</div>
          <div>
        ...
        <div style={{ display:"flex", gap:".4rem", alignItems:"center" }}>
          ...
          <button onClick={() => navigate("/onboard")} style={{ background:"#12487a", color:"#fff", border:"none", padding:".35rem .6rem", borderRadius:"8px", fontSize:".72rem", fontWeight:600, cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap" }}>
            + Qeydiyyat
          </button>
        </div>
      </header>

      <div style={{ background:"linear-gradient(135deg,#0a2d4a 0%,#12487a 60%,#1a5c8a 100%)", color:"#fff", padding:"3rem 1.2rem 2.5rem", textAlign:"center" }}>
        <div style={{ display:"inline-block", background:"rgba(255,255,255,.1)", border:"1px solid rgba(255,255,255,.18)", borderRadius:"20px", padding:".35rem 1rem", fontSize:".7rem", letterSpacing:".15em", textTransform:"uppercase", color:"#a8d4e8", marginBottom:"1.2rem" }}>✦ Bakının ən yaxşı restoranları</div>
        <h1 style={{ fontSize:"clamp(2rem,5vw,3.8rem)", fontWeight:800, lineHeight:1.1, letterSpacing:"-.03em", marginBottom:"1rem" }}>
          Masanızı indi<br/><span style={{ color:"#78b8d4", fontStyle:"italic" }}>rezerv edin</span>
        </h1>
        <p style={{ color:"#a8c8dc", fontSize:"1rem", maxWidth:"440px", margin:"0 auto", lineHeight:1.6 }}>
          Restoranı seçin, masanı rezerv edin, menyudan əvvəlcədən sifariş verin.
        </p>
        <div style={{ display:"flex", justifyContent:"center", gap:"2rem", marginTop:"1.5rem", flexWrap:"wrap" }}>
          {[["📍","24 restoran"],["🪑","Bu gün 140+ masa boşdur"],["⏱️","2 dəq ərzində təsdiq"]].map(([icon,text])=>(
            <div key={text} style={{ fontSize:".75rem", color:"#a8c8dc", display:"flex", alignItems:"center", gap:".3rem" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:"1rem", padding:"1rem 1rem 2rem", maxWidth:"1200px", margin:"0 auto" }}>
        ...
        {filtered.map(r => (
          <div key={r.id} className="rcard"
            style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:"14px", overflow:"hidden", opacity:r.available?1:0.6, boxShadow:"0 1px 4px rgba(0,0,0,.05)" }}
            ...
            {r.cover_url
              ? <div style={{ height:"140px", overflow:"hidden", position:"relative" }}>
                  ...
                  {!r.available && <span style={{ position:"absolute", top:"8px", right:"8px", fontSize:".62rem", padding:".2rem .55rem", background:"#FEF2F2", color:"#DC2626", borderRadius:"20px", fontWeight:600 }}>Tam dolu</span>}
                  <div style={{ position:"absolute", bottom:"8px", left:"10px", display:"flex", gap:".3rem" }}>
                    {r.tags.slice(0,2).map(t => <span key={t} style={{ fontSize:".58rem", padding:".15rem .45rem", borderRadius:"20px", background:"rgba(255,255,255,.88)", color:"#12487a", fontWeight:600 }}>{t}</span>)}
                  </div>
                </div>
              : <div style={{ height:"5px", background:r.accent||"#12487a" }}/>
            }
            <div style={{ padding:"1.2rem 1.4rem 1.4rem" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:".8rem" }}>
                <div style={{ width:"44px", height:"44px", borderRadius:"10px", background:`${r.accent||"#12487a"}15`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.3rem" }}>{r.emoji||"🍽️"}</div>
                {!r.available && !r.cover_url
              ...
              <div style={{ fontWeight:700, fontSize:"1.15rem", marginBottom:".2rem" }}>{r.name}</div>
              <div style={{ fontSize:".72rem", color:"#94A3B8", marginBottom:".6rem", textTransform:"uppercase", letterSpacing:".08em" }}>{r.cuisine}</div>
              <div style={{ display:"flex", alignItems:"center", gap:".4rem", marginBottom:".4rem" }}>
                <span style={{ color:"#F59E0B" }}>★</span>
                <span style={{ fontWeight:700, fontSize:".9rem" }}>{r.rating}</span>
              </div>
              <div style={{ fontSize:".76rem", color:"#64748B", marginBottom:r.cover_url?".8rem":"1rem" }}>📍 {r.location}</div>
              {!r.cover_url && <div style={{ display:"flex", flexWrap:"wrap", gap:".3rem", marginBottom:"1rem" }}>
                {r.tags.map(t => <span key={t} style={{ fontSize:".65rem", padding:".2rem .55rem", borderRadius:"20px", background:`${r.accent||"#12487a"}10`, color:r.accent||"#12487a", border:`1px solid ${r.accent||"#12487a"}25`, fontWeight:500 }}>{t}</span>)}
              </div>}
              {r.available && (
                <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:".65rem", borderRadius:"8px", background:"#12487a", color:"#fff", fontSize:".82rem", fontWeight:600 }}>
                  Rezervasiya et →
                </div>
              )}
            </div>
