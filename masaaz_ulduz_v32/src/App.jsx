  ...
  return (
    <div style={{ ...f, background:"#eef3f8", minHeight:"100vh", color:"#12487a" }}>
      ...
      <div style={{ maxWidth:"800px", margin:"0 auto", padding:"2rem" }}>
        ...
        {/* ══ PREMIUM MAP + INFO BLOCK ══ */}
        {(rest.phone || rest.hours || rest.location || rest.lat) && (()=>{
          // İş saatları hesablaması
          const nowD = new Date();
          ...
          // Koordinat varsa dəqiq, yoxsa ünvan mətni ilə
          const hasCoords = rest.lat && rest.lng;
          const mapsQ = hasCoords
            ? `${rest.lat},${rest.lng}`
            : encodeURIComponent((rest.location||rest.name)+", Bakı, Azərbaycan");
          const navUrl = hasCoords
            ? `https://www.google.com/maps/dir/?api=1&destination=${rest.lat},${rest.lng}`
            : `https://www.google.com/maps/dir/?api=1&destination=${mapsQ}`;
          const embedUrl = hasCoords
            ? `https://www.openstreetmap.org/export/embed.html?bbox=${parseFloat(rest.lng)-.008},${parseFloat(rest.lat)-.008},${parseFloat(rest.lng)+.008},${parseFloat(rest.lat)+.008}&layer=mapnik&marker=${rest.lat},${rest.lng}`
            : `https://maps.google.com/maps?q=${mapsQ}&output=embed&z=16&hl=az`;
          const showMap = !!(rest.location || hasCoords);

          return (
            <div style={{ marginBottom:"2rem", borderRadius:"24px", overflow:"hidden", boxShadow:"0 8px 40px rgba(0,0,0,.1)", border:"1px solid rgba(0,0,0,.06)" }}>
              ...
              {showMap && (
                <div style={{ position:"relative", height:"300px", overflow:"hidden" }}>
                  <iframe
                    ...
                    style={{ border:0, display:"block", filter:"saturate(1.1) contrast(1.05)" }}
                    loading="lazy" allowFullScreen
                    src={embedUrl}
                  />
                  {/* Dark vignette overlay at bottom */}
                  <div style={{ position:"absolute", bottom:0, left:0, right:0, height:"120px", background:"linear-gradient(to top, rgba(10,21,35,.85) 0%, transparent 100%)", pointerEvents:"none" }}/>

                  {/* Restoran marker pin */}
                  <div style={{ position:"absolute", bottom:"20px", left:"50%", transform:"translateX(-50%)", display:"flex", flexDirection:"column", alignItems:"center", pointerEvents:"none" }}>
                    <div style={{ background:"#0a1f35", color:"#fff", borderRadius:"12px", padding:".45rem 1rem", fontSize:".8rem", fontWeight:700, whiteSpace:"nowrap", boxShadow:"0 4px 16px rgba(0,0,0,.35)", display:"flex", alignItems:"center", gap:".4rem" }}>
                      <span style={{ fontSize:"1rem" }}>{rest.emoji||"🍽️"}</span>
                      {rest.name}
                    </div>
                    <div style={{ width:0, height:0, borderLeft:"6px solid transparent", borderRight:"6px solid transparent", borderTop:"8px solid #0a1f35", marginTop:"-1px" }}/>
                  </div>

                  {/* Open/Closed badge top-right */}
                  {isOpen !== null && (
                    <div style={{ position:"absolute", top:"14px", right:"14px", display:"flex", alignItems:"center", gap:".4rem", background: isOpen?"rgba(22,163,74,.92)":"rgba(220,38,38,.9)", color:"#fff", borderRadius:"100px", padding:".35rem .85rem", fontSize:".75rem", fontWeight:700, backdropFilter:"blur(8px)", boxShadow:"0 2px 12px rgba(0,0,0,.2)" }}>
                      <span style={{ width:"6px", height:"6px", borderRadius:"50%", background:"#fff", display:"inline-block", animation: isOpen?"pulse 1.8s ease infinite":"none" }}/>
                      {isOpen ? "Açıqdır" : "Bağlıdır"} · {rest.hours}
                    </div>
                  )}
                </div>
              ...
              {/* ── INFO ROW ── */}
              <div style={{ background:"#0a1f35", padding:"1.1rem 1.4rem", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:".8rem" }}>

                {/* Left: location + hours + phone */}
                <div style={{ display:"flex", flexDirection:"column", gap:".5rem", flex:1, minWidth:"200px" }}>
                  {rest.location && (
                    <div style={{ display:"flex", alignItems:"center", gap:".55rem" }}>
                      <span style={{ fontSize:".85rem", flexShrink:0 }}>📍</span>
                      <span style={{ fontSize:".82rem", color:"rgba(255,255,255,.8)", fontWeight:500 }}>{rest.location}, Bakı</span>
                    </div>
                  )}
                  {rest.hours && (
                    <div style={{ display:"flex", alignItems:"center", gap:".55rem" }}>
                      <span style={{ fontSize:".85rem", flexShrink:0 }}>🕐</span>
                      <span style={{ fontSize:".82rem", color:"rgba(255,255,255,.65)" }}>{rest.hours}</span>
                      {isOpen !== null && <span style={{ fontSize:".68rem", fontWeight:700, color:isOpen?"#4ade80":"#f87171" }}>{isOpen?"● Açıqdır":"● Bağlıdır"}</span>}
                    </div>
                  )}
                  {rest.phone && (
                    <div style={{ display:"flex", alignItems:"center", gap:".55rem" }}>
                      <span style={{ fontSize:".85rem", flexShrink:0 }}>📞</span>
                      <a href={"tel:"+rest.phone} style={{ fontSize:".82rem", color:"rgba(255,255,255,.8)", fontWeight:600, textDecoration:"none" }}>{rest.phone}</a>
                    </div>
                  )}
                </div>
                ...
                <div style={{ display:"flex", gap:".6rem", flexWrap:"wrap" }}>
                  ...
                  {showMap && (
                    <a href={navUrl} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:".5rem", background:"linear-gradient(135deg,#c8a96e,#e0c07a)", color:"#0a1f35", borderRadius:"12px", padding:".7rem 1.3rem", fontSize:".85rem", fontWeight:800, textDecoration:"none", whiteSpace:"nowrap", boxShadow:"0 4px 16px rgba(200,169,110,.35)", transition:"all .2s", letterSpacing:"-.01em" }}
                      onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(200,169,110,.4)";}}
                      onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 4px 16px rgba(200,169,110,.35)";}}>
                  ...
                  {rest.phone && (
                    <a href={"tel:"+rest.phone}
                      style={{ display:"flex", alignItems:"center", gap:".5rem", background:"rgba(255,255,255,.1)", border:"1px solid rgba(255,255,255,.15)", color:"#fff", borderRadius:"12px", padding:".7rem 1.3rem", fontSize:".85rem", fontWeight:700, textDecoration:"none", whiteSpace:"nowrap", backdropFilter:"blur(8px)", transition:"all .2s" }}
                  ...
                  {/* Open in Maps */}
                  {showMap && (
                    <a href={hasCoords ? `https://maps.google.com?q=${rest.lat},${rest.lng}` : "https://maps.google.com?q="+mapsQ} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:".5rem", background:"rgba(255,255,255,.08)", border:"1px solid rgba(255,255,255,.12)", color:"rgba(255,255,255,.7)", borderRadius:"12px", padding:".7rem 1rem", fontSize:".82rem", fontWeight:600, textDecoration:"none", whiteSpace:"nowrap", transition:"all .2s" }}
